---
name: node-connect
description: Diagnose AvaClaw node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify AvaClaw is advertising that route, then fix pairing/auth.
...
- whether `avaclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `avaclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
avaclaw config get gateway.mode
avaclaw config get gateway.bind
avaclaw config get gateway.tailscale.mode
avaclaw config get gateway.remote.url
avaclaw config get gateway.auth.mode
avaclaw config get gateway.auth.allowTailscale
avaclaw config get plugins.entries.device-pair.config.publicUrl
avaclaw qr --json
avaclaw devices list
avaclaw nodes status
```

If this AvaClaw instance is pointed at a remote gateway, also run:

```bash
avaclaw qr --remote --json
```
...
If `avaclaw qr --json` says `Gateway is only bound to loopback`:
...
```bash
avaclaw devices list
avaclaw devices approve --latest
```
...
- `avaclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run avaclaw qr again, rescan, then approve the pending device pairing.`

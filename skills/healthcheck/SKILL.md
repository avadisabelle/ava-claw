---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for AvaClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, AvaClaw cron scheduling for periodic checks, or version status checks on a machine running AvaClaw (laptop, workstation, Pi, VPS).
---

# AvaClaw Host Hardening

## Overview

Assess and harden the host running AvaClaw, then align it to a user-defined risk tolerance without breaking access. Use AvaClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim AvaClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. AvaClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if AvaClaw can access sensitive data.
...
### 2) Run AvaClaw security audits (read-only)

As part of the default read-only checks, run `avaclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `avaclaw security audit` (faster, non-probing)
2. `avaclaw security audit --json` (structured output)

Offer to apply AvaClaw safe defaults (numbered):

1. `avaclaw security audit --fix`

Be explicit that `--fix` only tightens AvaClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check AvaClaw version/update status (read-only)

As part of the default read-only checks, run `avaclaw update status`.
...
- Credential hygiene notes (location of AvaClaw creds, prefer disk encryption)
...
After AvaClaw install or first hardening pass, run at least one baseline audit and version check:

- `avaclaw security audit`
- `avaclaw security audit --deep`
- `avaclaw update status`

Ongoing monitoring is recommended. Use the AvaClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `avaclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `avaclaw update status`
...
Before creating, `avaclaw cron list` and match on exact `name`. If found, `avaclaw cron edit <id> ...`.
If not found, `avaclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `avaclaw update status` (preferred for source checkouts and channels)
2. `npm view avaclaw version` (published npm version)

## AvaClaw command accuracy

Use only supported commands and flags:

- `avaclaw security audit [--deep] [--fix] [--json]`
- `avaclaw status` / `avaclaw status --deep`
- `avaclaw health --json`
- `avaclaw update status`
- `avaclaw cron add|list|runs|run`

Do not invent CLI flags or imply AvaClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by AvaClaw compaction:

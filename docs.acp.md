...
This document describes how the AvaClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`avaclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
AvaClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing AvaClaw Gateway
...
- ACP client spawns `avaclaw acp` and speaks ACP messages over stdio.
...
`avaclaw acp` resolves the Gateway URL and auth from CLI flags or config:

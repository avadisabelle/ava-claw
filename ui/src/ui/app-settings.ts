...
import type { AvaClawApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as AvaClawApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as AvaClawApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as AvaClawApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as AvaClawApp);
    await loadConfig(host as unknown as AvaClawApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as AvaClawApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as AvaClawApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as AvaClawApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as AvaClawApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as AvaClawApp);
    await loadDevices(host as unknown as AvaClawApp);
    await loadConfig(host as unknown as AvaClawApp);
    await loadExecApprovals(host as unknown as AvaClawApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as AvaClawApp);
    await loadConfig(host as unknown as AvaClawApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as AvaClawApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as AvaClawApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__AVACLAW_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as AvaClawApp;
...
async function loadOverviewLogs(host: AvaClawApp) {
...
function buildAttentionItems(host: AvaClawApp) {
  ...
  if (auth?.scopes && !hasOperatorReadAccess(auth)) {
    items.push({
      ...
      href: "https://docs.avaclaw.ai/web/dashboard",
      external: true,
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as AvaClawApp, true),
    loadConfigSchema(host as unknown as AvaClawApp),
    loadConfig(host as unknown as AvaClawApp),
  ]);
}

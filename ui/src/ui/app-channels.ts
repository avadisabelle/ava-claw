...
export async function handleWhatsAppStart(host: AvaClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: AvaClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: AvaClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: AvaClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: AvaClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: AvaClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: AvaClawApp): string | null {
...
function buildGatewayHttpHeaders(host: AvaClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: AvaClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: AvaClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: AvaClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: AvaClawApp) {
  const state = host.nostrProfileFormState;

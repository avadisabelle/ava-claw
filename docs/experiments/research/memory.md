...
read_when:
  - Designing workspace memory (~/.avaclaw/workspace) beyond daily Markdown logs
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.avaclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.avaclaw/workspace` as canonical human-readable memory.
...
```
~/.avaclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Ava-Claw**, but keep a separable core library.

### Why integrate into Ava-Claw?

- Ava-Claw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `avaclaw memory recall "…" --k 25 --since 30d`
  - `avaclaw memory reflect --since 7d`
...
Pragmatic take for `~/.avaclaw/workspace`:

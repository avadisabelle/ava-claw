...
This plan defines how AvaClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- AvaClaw core owns ACP control plane concerns
...
AvaClaw should not reimplement ACP transport internals in core.
AvaClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in AvaClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under AvaClaw state dir
...
- ACP event stream is normalized to AvaClaw reply chunks
...
For the first production backend (`extensions/acpx`), AvaClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses AvaClaw `sessionKey`)
...
- AvaClaw consumes ndjson events from `acpx --format json --json-strict`

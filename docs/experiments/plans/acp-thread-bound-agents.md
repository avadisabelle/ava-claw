...
This plan defines how Ava-Claw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Ava-Claw core owns ACP control plane concerns
...
Ava-Claw should not reimplement ACP transport internals in core.
Ava-Claw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Ava-Claw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Ava-Claw state dir
...
- ACP event stream is normalized to Ava-Claw reply chunks
...
For the first production backend (`extensions/acpx`), Ava-Claw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Ava-Claw `sessionKey`)
...
- Ava-Claw consumes ndjson events from `acpx --format json --json-strict`

...
```bash
avaclaw plugins enable minimax-portal-auth  # skip if already loaded.
avaclaw gateway restart  # restart if gateway is already running
...
Configure manually via `avaclaw.json`:
...
- Use `avaclaw models list` and `avaclaw models set minimax/MiniMax-M2.5` to switch.
...
- Running `avaclaw configure` and selecting **MiniMax M2.5**, or

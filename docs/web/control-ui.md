...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/avaclaw`)
...
If the page fails to load, start the Gateway first: `avaclaw gateway`.
...
you revoke it with `avaclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.avadisabelle/ava-claw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop avaclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Ava-Claw
...
Ava-Claw **blocks** Control UI connections without device identity.

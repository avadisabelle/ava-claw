...
```bash
avaclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `avaclaw tui --deliver`
...
- Local shell commands receive `AVACLAW_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `AVACLAW_THEME=light` before launching `avaclaw tui`.
- To force the original dark palette instead, set `AVACLAW_THEME=dark`.
...
- Check the Gateway logs: `avaclaw logs --follow`.
- Confirm the agent can run: `avaclaw status` and `avaclaw models status`.
...
- No agents in picker: check `avaclaw agents list` and your routing config.

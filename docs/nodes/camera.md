...
Ava-Claw supports **camera capture** for agent workflows:
...
```bash
avaclaw nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
avaclaw nodes camera snap --node <id> --facing front
avaclaw nodes camera clip --node <id> --duration 3000
avaclaw nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`avaclaw.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `avaclaw` CLI to invoke camera commands on the macOS node.

Examples:

```bash
avaclaw nodes camera list --node <id>            # list camera ids
avaclaw nodes camera snap --node <id>            # prints MEDIA:<path>
avaclaw nodes camera snap --node <id> --max-width 1280
avaclaw nodes camera snap --node <id> --delay-ms 2000
avaclaw nodes camera snap --node <id> --device-id <id>
avaclaw nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
avaclaw nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
avaclaw nodes camera clip --node <id> --device-id <id>
avaclaw nodes camera clip --node <id> --no-audio
```

Notes:

- `avaclaw nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
avaclaw nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```

...
fly apps create my-avaclaw

# Create a persistent volume (1GB is usually enough)
fly volumes create avaclaw_data --size 1 --region iad
```
...
[env]
  NODE_ENV = "production"
  AVACLAW_PREFER_PNPM = "1"
  AVACLAW_STATE_DIR = "/data"
...
| `internal_port = 3000`         | Must match `--port 3000` (or `AVACLAW_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
| `AVACLAW_STATE_DIR = "/data"` | Persists state on the volume                                                |

## 3) Set secrets

```bash
# Required: Gateway token (for non-loopback binding)
fly secrets set AVACLAW_GATEWAY_TOKEN=$(openssl rand -hex 32)
...
- Non-loopback binds (`--bind lan`) require `AVACLAW_GATEWAY_TOKEN` for security.
- Treat these tokens like passwords.
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `avaclaw.json` where they could be accidentally exposed or logged.
...
**Note:** With `AVACLAW_STATE_DIR=/data`, the config path is `/data/avaclaw.json`.
...
Or visit `https://my-avaclaw.fly.dev/`

Paste your gateway token (the one from `AVACLAW_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `AVACLAW_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/avaclaw.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/avaclaw.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/avaclaw.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/avaclaw.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/avaclaw.json"
```
...
**Fix:** Ensure `AVACLAW_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-avaclaw

# Release public IPs
fly ips release <public-ipv4> -a my-avaclaw
fly ips release <public-ipv6> -a my-avaclaw
...
fly ips allocate-v6 --private -a my-avaclaw
```
...
fly proxy 3000:3000 -a my-avaclaw

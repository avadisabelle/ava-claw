...
This also creates a minimal `~openclaw/.avadisabelle/ava-claw.json` (sets `gateway.mode="local"`) so the gateway can start without running the wizard.
...
- **Config:** Only `openclaw` and root can access `/home/openclaw/.openclaw`. To edit config: use the Control UI once the gateway is running, or `sudo -u openclaw $EDITOR /home/openclaw/.avadisabelle/ava-claw.json`.
...
- **Gateway start blocked (missing `gateway.mode=local`):** Ensure `~openclaw/.avadisabelle/ava-claw.json` exists and sets `gateway.mode="local"`. `setup-podman.sh` creates this file if missing.

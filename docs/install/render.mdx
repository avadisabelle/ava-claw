...
Deploy AvaClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/avaclaw/avaclaw)
...
repository configures everything needed to run AvaClaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: AVACLAW_STATE_DIR
        value: /data/.avaclaw
      - key: AVACLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: AVACLAW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/avaclaw`.
...
If you use the original AvaClaw repository, Render will not auto-deploy your AvaClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For AvaClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any AvaClaw host.

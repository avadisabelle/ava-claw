...
Deploy AvaClaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/avaclaw` — Control UI
...
- `AVACLAW_STATE_DIR=/data/.avaclaw` (recommended)
- `AVACLAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `AVACLAW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your AvaClaw state + workspace so you can migrate to another host without losing config or memory.

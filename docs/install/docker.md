...
- builds the gateway image locally (or pulls a remote image if `AVACLAW_IMAGE` is set)
...
- `AVACLAW_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/avadisabelle/ava-claw:latest`)
- `AVACLAW_DOCKER_APT_PACKAGES` — install extra apt packages during build
- `AVACLAW_EXTENSIONS` — pre-install extension dependencies at build time (space-separated extension names, e.g. `diagnostics-otel matrix`)
- `AVACLAW_EXTRA_MOUNTS` — add extra host bind mounts
- `AVACLAW_HOME_VOLUME` — persist `/home/node` in a named volume
- `AVACLAW_SANDBOX` — opt in to Docker gateway sandbox bootstrap. Only explicit truthy values enable it: `1`, `true`, `yes`, `on`
- `AVACLAW_INSTALL_DOCKER_CLI` — build arg passthrough for local image builds (`1` installs Docker CLI in the image). `docker-setup.sh` sets this automatically when `AVACLAW_SANDBOX=1` for local builds.
- `AVACLAW_DOCKER_SOCKET` — override Docker socket path (default: `DOCKER_HOST=unix://...` path, else `/var/run/docker.sock`)
- `AVACLAW_ALLOW_INSECURE_PRIVATE_WS=1` — break-glass: allow trusted private-network
  `ws://` targets for CLI/onboarding client paths (default is loopback-only)
- `AVACLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` — disable container browser hardening flags
  `--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu` when you need
  WebGL/3D compatibility.
- `AVACLAW_BROWSER_DISABLE_EXTENSIONS=0` — keep extensions enabled when browser
  flows require them (default keeps extensions disabled in sandbox browser).
- `AVACLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>` — set Chromium renderer process
...
- Need the URL again? Run `docker compose run --rm avaclaw-cli dashboard --no-open`.
...
```bash
export AVACLAW_SANDBOX=1
export AVACLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
./docker-setup.sh
```
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `avaclaw-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
- For non-local `AVACLAW_IMAGE` values, the image must already contain Docker
...
```bash
docker compose run -T --rm avaclaw-cli gateway probe
docker compose run -T --rm avaclaw-cli devices list --json
```
...
`avaclaw-cli` uses `network_mode: "service:avaclaw-gateway"` so CLI commands can
...
network path instead of the bundled `avaclaw-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `avaclaw-cli`.
...
- [GitHub Container Registry package](https://github.com/avadisabelle/ava-claw/pkgs/container/avaclaw)

Use image name `ghcr.io/avadisabelle/ava-claw` (not similarly named Docker Hub
images).
...
- `org.opencontainers.image.source=https://github.com/avadisabelle/ava-claw`
...
image instead, set `AVACLAW_IMAGE` before running the script:

```bash
export AVACLAW_IMAGE="ghcr.io/avadisabelle/ava-claw:latest"
./docker-setup.sh
```

The script detects that `AVACLAW_IMAGE` is not the default `avaclaw:local` and
...
`docker-compose.yml` and helper files. `AVACLAW_IMAGE` skips local image build
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/avadisabelle/ava-claw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```
...
See [`ClawDock` Helper README](https://github.com/avadisabelle/ava-claw/blob/main/scripts/shell-helpers/README.md) for details.

### Manual flow (compose)

```bash
docker build -t avaclaw:local -f Dockerfile .
docker compose run --rm avaclaw-cli onboard
docker compose up -d avaclaw-gateway
```

Note: run `docker compose ...` from the repo root. If you enabled
`AVACLAW_EXTRA_MOUNTS` or `AVACLAW_HOME_VOLUME`, the setup script writes
...
```bash
docker compose run --rm avaclaw-cli dashboard --no-open
docker compose run --rm avaclaw-cli devices list
docker compose run --rm avaclaw-cli devices approve <requestId>
```
...
`AVACLAW_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
`avaclaw-gateway` and `avaclaw-cli` by generating `docker-compose.extra.yml`.

Example:

```bash
export AVACLAW_EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
- If you edit `AVACLAW_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
volume via `AVACLAW_HOME_VOLUME`. This creates a Docker volume and mounts it at
`/home/node`, while keeping the standard config/workspace bind mounts. Use a
named volume here (not a bind path); for bind mounts, use
`AVACLAW_EXTRA_MOUNTS`.

Example:

```bash
export AVACLAW_HOME_VOLUME="avaclaw_home"
./docker-setup.sh
```
...
- If you change `AVACLAW_HOME_VOLUME`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
libraries), set `AVACLAW_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
```bash
export AVACLAW_DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
- If you change `AVACLAW_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
dependencies into the image instead, set `AVACLAW_EXTENSIONS` before
running `docker-setup.sh`:

```bash
export AVACLAW_EXTENSIONS="diagnostics-otel matrix"
./docker-setup.sh
```
...
```bash
docker build --build-arg AVACLAW_EXTENSIONS="diagnostics-otel matrix" .
```
...
- If you change `AVACLAW_EXTENSIONS`, rerun `docker-setup.sh` to rebuild
  the image.
...
```bash
export AVACLAW_DOCKER_APT_PACKAGES="git curl jq"
./docker-setup.sh
```
...
`AVACLAW_DOCKER_APT_PACKAGES` instead of using `--with-deps` at runtime.

4. **Persist Playwright browser downloads**:

- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `AVACLAW_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `AVACLAW_EXTRA_MOUNTS`.

### Permissions + EACCES

The image runs as `node` (uid 1000). If you see permission errors on
`/home/node/.avaclaw`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/avaclaw-config /path/to/avaclaw-workspace
```
...
```bash
docker compose run --rm avaclaw-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm avaclaw-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm avaclaw-cli channels add --channel discord --token "<token>"
```
...
```bash
docker compose exec avaclaw-gateway node dist/index.js health --token "$AVACLAW_GATEWAY_TOKEN"
```
...
`docker-setup.sh` defaults `AVACLAW_GATEWAY_BIND=lan` so host access to
...
```bash
docker compose run --rm avaclaw-cli config set gateway.mode local
docker compose run --rm avaclaw-cli config set gateway.bind lan
docker compose run --rm avaclaw-cli devices list --url ws://127.0.0.1:18789
```

### Notes

- Gateway bind defaults to `lan` for container use (`AVACLAW_GATEWAY_BIND`).
- Dockerfile CMD uses `--allow-unconfigured`; mounted config with `gateway.mode` not `local` will still start. Override CMD to enforce the guard.
- The gateway container is the source of truth for sessions (`~/.avaclaw/agents/<agentId>/sessions/`).
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.avaclaw/sandboxes`
...
This builds `avaclaw-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `avaclaw-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "avaclaw-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `avaclaw-sandbox-browser:bookworm-slim` using
...
- Browser container startup defaults are conservative for shared/container workloads, including:
  ...
  - The three graphics hardening flags above are optional. If your workload needs
    WebGL/3D, set `AVACLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` to run without
  ...
  - Extension behavior is controlled by `--disable-extensions` and can be disabled
    (enables extensions) via `AVACLAW_BROWSER_DISABLE_EXTENSIONS=0` for
    extension-dependent pages or extensions-heavy workflows.
  - `--renderer-process-limit=2` is also configurable with
    `AVACLAW_BROWSER_RENDERER_PROCESS_LIMIT`; set `0` to let Chromium choose its
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-avaclaw-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-avaclaw-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-avaclaw-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/avadisabelle/ava-claw/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.

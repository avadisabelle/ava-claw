---
summary: "Frequently asked questions about Ava-Claw setup, configuration, and usage"
read_when:
...
### What's the recommended way to install and set up Ava-Claw
...
### How do I install Ava-Claw on Linux
...
### How do I install Ava-Claw on a VPS
...
### Can I ask Ava-Claw to update itself
...
No. You can run Ava-Claw with **API keys** (Anthropic/OpenAI/others) or with
...
OpenAI Codex OAuth is explicitly supported for external tools like Ava-Claw.
...
Yes - via **setup-token**. Ava-Claw no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so Ava-Claw can keep replying while a provider is rate-limited.
...
Ava-Claw supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. Ava-Claw fully supports **OpenAI Code (Codex) subscription OAuth**.
OpenAI explicitly allows subscription OAuth usage in external tools/workflows
like Ava-Claw. The onboarding wizard can run the OAuth flow for you.
...
Usually no. Ava-Claw needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. Ava-Claw runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run Ava-Claw can I connect it to my MacBook Pro
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but Ava-Claw authorization uses numeric IDs only.
...
### Can multiple people use one WhatsApp number with different Ava-Claw instances
...
If you run Ava-Claw via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
**Ava-Claw-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run Ava-Claw on a dedicated machine
...
Ava-Claw is lightweight. For a basic Gateway + one chat channel:
...
### Can I run Ava-Claw in a VM and what are the requirements
...
### What is Ava-Claw in one paragraph

Ava-Claw is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

Ava-Claw is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for Ava-Claw
...
### Can Ava-Claw help with lead gen outreach ads and blogs for a SaaS
...
Ava-Claw draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

Ava-Claw is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use Ava-Claw when you
...
### Can Ava-Claw run tasks on a schedule or continuously in the background
...
Ava-Claw memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

Ava-Claw also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, Ava-Claw auto-selects a provider when it
...
configured and present, Ava-Claw
...
### Is all data used with Ava-Claw saved locally
...
### How do I completely uninstall Ava-Claw
...
`workspace` to the repo root. The Ava-Claw repo is just source code; keep the
...
### Can the Ava-Claw browser run headless
...
### Can two Ava-Claw instances talk to each other local VPS
...
### How does Ava-Claw load environment variables
...
### Is there a way to make a team of Ava-Claw instances one CEO and many agents
...
### How do I completely reset Ava-Claw but keep it installed
...
headers like `# Heading`), Ava-Claw skips the heartbeat run to save API calls.
...
No. Ava-Claw runs on **your own account**, so if you're in the group, Ava-Claw can see it.
...
### Why doesn't Ava-Claw reply in a group
...
Ava-Claw's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, Ava-Claw currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
Yes. Ava-Claw ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
Cooldowns apply to failing profiles (exponential backoff), so Ava-Claw can keep responding even when a provider is rate-limited or temporarily failing.
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Ava-Claw will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: Ava-Claw now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
Ava-Claw uses provider-prefixed IDs like:
...
- `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Ava-Claw already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
...
### Is it safe to expose Ava-Claw to inbound DMs
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Ava-Claw only replies to chats it receives or to explicit sends you trigger.
...
Ava-Claw blocks **cross-provider** messaging by default. If a tool call is bound

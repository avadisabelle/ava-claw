...
This document outlines the design for integrating "Kilo Gateway" as a first-class provider in Ava-Claw, modeled after the existing OpenRouter implementation. Kilo Gateway uses an OpenAI-compatible completions API with a different base URL.
...
export function applyKilocodeProviderConfig(cfg: Ava-ClawConfig): Ava-ClawConfig {
...
export function applyKilocodeConfig(cfg: Ava-ClawConfig): Ava-ClawConfig {

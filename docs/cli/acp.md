...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a AvaClaw Gateway.
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the AvaClaw gateway or agent instead.                                                                                                                                      |
...
it to drive a AvaClaw Gateway session.
...
AvaClaw bot over ACP, use `acpx` with its built-in `avaclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx avaclaw` at `avaclaw acp`.
3. Target the AvaClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default AvaClaw ACP session
acpx avaclaw exec "Summarize the active AvaClaw session state."

# Persistent named session for follow-up turns
acpx avaclaw sessions ensure --name codex-bridge
acpx avaclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my AvaClaw work agent for recent context relevant to this repo."
```
...
For a repo-local AvaClaw checkout, use the direct CLI entrypoint instead of the
...
pull contextual information from an AvaClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select “AvaClaw ACP” to start a thread.

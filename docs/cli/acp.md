...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a Ava-Claw Gateway.
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Ava-Claw gateway or agent instead.                                                                                                                                     |
...
it to drive a Ava-Claw Gateway session.
...
For a repo-local Ava-Claw checkout, use the direct CLI entrypoint instead of the
...
pull contextual information from an Ava-Claw agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Ava-Claw ACP” to start a thread.

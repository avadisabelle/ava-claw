---
summary: "CLI reference for `avaclaw configure` (interactive configuration prompts)"
read_when:
...
Tip: `avaclaw config` without a subcommand opens the same wizard. Use
`avaclaw config get|set|unset` for non-interactive edits.
...
```bash
avaclaw configure
avaclaw configure --section model --section channels
```

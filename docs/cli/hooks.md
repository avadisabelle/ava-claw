---
summary: "CLI reference for `avaclaw hooks` (agent hooks)"
read_when:
...
Details:
  Source: avaclaw-bundled
  Path: /path/to/avaclaw/hooks/bundled/session-memory/HOOK.md
  Handler: /path/to/avaclaw/hooks/bundled/session-memory/handler.ts
...
```bash
avaclaw hooks enable <name>
```

Enable a specific hook by adding it to your config (`~/.avaclaw/config.json`).

**Note:** Hooks managed by plugins show `plugin:<id>` in `avaclaw hooks list` and
...
avaclaw hooks install <npm-spec> --pin
```
...
- Copies the hook pack into `~/.avaclaw/hooks/<id>`
...
```bash
# Local directory
avaclaw hooks install ./my-hook-pack
...
```bash
avaclaw hooks update <id>
avaclaw hooks update --all
```
...
**Output:** `~/.avaclaw/workspace/memory/YYYY-MM-DD-slug.md`

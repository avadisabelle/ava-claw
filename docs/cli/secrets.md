---
summary: "CLI reference for `avaclaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `avaclaw secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `avaclaw.json` refs)
...
```bash
avaclaw secrets audit
avaclaw secrets audit --check
avaclaw secrets audit --json
```
...
```bash
avaclaw secrets configure
avaclaw secrets configure --plan-out /tmp/avaclaw-secrets-plan.json
avaclaw secrets configure --apply --yes
avaclaw secrets configure --providers-only
avaclaw secrets configure --skip-provider-setup
avaclaw secrets configure --agent ops
avaclaw secrets configure --json
```
...
- `configure` targets secret-bearing fields in `avaclaw.json` plus `auth-profiles.json` for the selected agent scope.
...
avaclaw secrets apply --from /tmp/avaclaw-secrets-plan.json
avaclaw secrets apply --from /tmp/avaclaw-secrets-plan.json --dry-run
avaclaw secrets apply --from /tmp/avaclaw-secrets-plan.json --json
```
...
- `avaclaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.avaclaw/.env` known secret keys whose values were migrated

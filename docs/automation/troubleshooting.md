...
avaclaw system heartbeat last
avaclaw logs --follow
avaclaw config get agents.defaults.heartbeat
avaclaw channels status --probe
```
...
```bash
avaclaw config get agents.defaults.heartbeat.activeHours
avaclaw config get agents.defaults.heartbeat.activeHours.timezone
avaclaw config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
avaclaw cron list
avaclaw logs --follow
```

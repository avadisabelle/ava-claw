...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in AvaClaw.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in AvaClaw. See [Webhook Hooks](/automation/webhook) or use `avaclaw webhooks` for Gmail helper commands.
...
- Extend AvaClaw's behavior without modifying core code

## Getting Started

### Bundled Hooks

AvaClaw ships with four bundled hooks that are automatically discovered:
...
3. **Bundled hooks**: `<avaclaw>/dist/hooks/bundled/` (shipped with AvaClaw)
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before AvaClaw persists them.
...
- [Bundled Hooks README](https://github.com/avadisabelle/ava-claw/tree/main/src/hooks/bundled)

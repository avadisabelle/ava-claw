...
To reduce top-of-hour load spikes across many gateways, AvaClaw applies a
...
If `delivery` is omitted for isolated jobs, AvaClaw defaults to `announce`.
...
When a job fails, AvaClaw classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, AvaClaw archives transcript files and purges old deleted archives on the same retention window.
...
- AvaClaw applies exponential retry backoff for recurring jobs after consecutive errors:

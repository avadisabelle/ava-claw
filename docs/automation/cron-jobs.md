...
- Jobs persist under `~/.avaclaw/cron/` so restarts don’t lose schedules.
...
- For upgrades, `avaclaw doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
avaclaw cron list
avaclaw cron run <job-id>
avaclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.avaclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `avaclaw cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Ava-Claw applies a
...
If `delivery` is omitted for isolated jobs, Ava-Claw defaults to `announce`.
...
- CLI equivalents: `avaclaw cron add --light-context ...` and `avaclaw cron edit --light-context`.
...
- Job store: `~/.avaclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.avaclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Ava-Claw classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Ava-Claw archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `avaclaw cron runs` and adjust retention before logs become large
...
avaclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
avaclaw cron edit <jobId> --agent ops
avaclaw cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
avaclaw cron run <jobId>
avaclaw cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `avaclaw cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
avaclaw cron edit <jobId> \
...
```bash
avaclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`avaclaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `AVACLAW_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Ava-Claw applies exponential retry backoff for recurring jobs after consecutive errors:

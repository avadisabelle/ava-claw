...
read_when:
  - 添加或修改插件/扩展
  - 记录插件安装或加载规则
summary: AvaClaw 插件/扩展：发现、配置和安全
title: 插件
x-i18n:
...
插件只是一个**小型代码模块**，用额外功能（命令、工具和 Gateway 网关 RPC）扩展 AvaClaw。

大多数时候，当你想要一个尚未内置到核心 AvaClaw 的功能（或你想将可选功能排除在主安装之外）时，你会使用插件。
...
- 从 2026.1.15 起 Microsoft Teams 仅作为插件提供；如果使用 Teams，请安装 `ava-claw-msteams`。
...
AvaClaw 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。**配置验证不会执行插件代码**；它使用插件清单和 JSON Schema。参见 [插件清单](/plugins/manifest)。
...
AvaClaw 还可以合并**外部渠道目录**（例如，MPM 注册表导出）。将 JSON 文件放在以下位置之一：
...
如果插件导出 `id`，AvaClaw 会使用它，但当它与配置的 id 不匹配时会发出警告。
...
插件可以注册**模型提供商认证**流程，以便用户可以在 AvaClaw 内运行 OAuth 或 API 密钥设置（无需外部脚本）。
...
- 主包：`avaclaw`（本仓库）
- 插件：`ava-claw-*` 下的独立 npm 包（例如：`ava-claw-voice-call`）

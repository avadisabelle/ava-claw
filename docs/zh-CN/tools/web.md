...
**推荐：** 运行 `avaclaw configure --section web`。它将密钥存储在 `~/.avaclaw/avaclaw.json` 的 `tools.web.search.apiKey` 下。
...
如果未设置基础 URL，AvaClaw 会根据 API 密钥来源选择默认值：

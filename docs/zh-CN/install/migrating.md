...
- 复制**状态目录**（`$AVACLAW_STATE_DIR`，默认：`~/.avaclaw/`）— 这包括配置、认证、会话和渠道状态。
- 复制你的**工作区**（默认 `~/.avaclaw/workspace/`）— 这包括你的智能体文件（记忆、提示等）。
...
- `--profile <name>`（通常变成 `~/.avaclaw-<profile>/`）
...
在输出中查找 `AVACLAW_STATE_DIR` / profile 的提及。如果你运行多个 Gateway 网关，对每个配置文件重复此操作。

### 2）确定你的工作区

常见默认值：

- `~/.avaclaw/workspace/`（推荐的工作区）
- 你创建的自定义文件夹
...
在这个阶段，如果新手引导创建了一个新的 `~/.avaclaw/` 也没关系 — 你将在下一步覆盖它。
...
如果你在旧 Gateway 网关上使用了配置文件（或 `AVACLAW_STATE_DIR`），而新 Gateway 网关使用了不同的配置，你会看到如下症状：
...
`$AVACLAW_STATE_DIR` 包含密钥（API 密钥、OAuth 令牌、WhatsApp 凭证）。将备份视为生产密钥：
...
- `avaclaw status` 显示 Gateway 网关正在运行

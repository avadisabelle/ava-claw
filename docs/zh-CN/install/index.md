...
pnpm approve-builds -g                # 批准 avaclaw、node-llama-cpp、sharp 等
pnpm add -g avaclaw@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm avaclaw ...` 运行仓库命令。
...
- 运行新手引导：`avaclaw onboard --install-daemon`
- 快速检查：`avaclaw doctor`
- 检查 Gateway 网关健康状态：`avaclaw status` + `avaclaw health`
- 打开仪表板：`avaclaw dashboard`
...
```bash
# 显式 npm
curl -fsSL https://avaclaw.ai/install.sh | bash -s -- --install-method npm
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `avaclaw`）。

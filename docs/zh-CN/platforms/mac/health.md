...
- 你仍然可以使用 [Gateway 网关健康](/gateway/health) 中的 CLI 流程（`openclaw status`、`openclaw status --deep`、`openclaw health --json`），并在 `/tmp/avadisabelle/ava-claw-*.log` 中跟踪 `web-heartbeat` / `web-reconnect`。

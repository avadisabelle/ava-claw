---
read_when:
  - 在 DigitalOcean 上设置 Ava-Claw
  - 寻找便宜的 VPS 托管来运行 Ava-Claw
summary: 在 DigitalOcean 上运行 Ava-Claw（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 Ava-Claw

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 Ava-Claw Gateway 网关。
...
# Check service
systemctl --user status avaclaw-gateway.service

# View logs
journalctl --user -u avaclaw-gateway.service -f
```
...
```bash
avaclaw channels login whatsapp
# Scan QR code
```
...
- `~/.avaclaw/` — 配置、凭证、会话数据
- `~/.avaclaw/workspace/` — 工作区（SOUL.md、记忆等）
...
journalctl -u avaclaw --no-pager -n 50
```

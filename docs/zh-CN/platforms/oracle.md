...
这会启用 Tailscale SSH，所以你可以从 tailnet 上的任何设备通过 `ssh avaclaw` 连接——不需要公网 IP。
...
此设置通常消除了纯粹为了阻止互联网范围的 SSH 暴力破解而需要额外的基于主机的防火墙规则的*需求*——但你仍应保持操作系统更新，运行 `avaclaw security audit`，并验证你没有意外地在公共接口上监听。
...
- **凭证权限：** `chmod 700 ~/.avaclaw`
- **安全审计：** `avaclaw security audit`
...
- `~/.avaclaw/` — 配置、凭证、会话数据
- `~/.avaclaw/workspace/` — 工作区（SOUL.md、记忆、产物）

...
read_when:
  ...
  - 你想在 Ava-Claw 中通过 OpenRouter 运行模型
summary: 使用 OpenRouter 的统一 API 在 Ava-Claw 中访问多种模型
title: OpenRouter
x-i18n:
...
```bash
avaclaw onboard --auth-choice apiKey --token-provider openrouter --token "$OPENROUTER_API_KEY"
```

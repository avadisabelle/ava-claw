...
```bash
avaclaw plugins enable minimax-portal-auth  # 如果已加载则跳过
avaclaw gateway restart  # 如果 Gateway 网关已在运行则重启
avaclaw onboard --auth-choice minimax-portal
```
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/avadisabelle/ava-claw/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `avaclaw models list` 和 `avaclaw models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `avaclaw configure` 并选择 **MiniMax M2.1**，或

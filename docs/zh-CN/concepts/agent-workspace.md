...
当启用沙箱隔离且 `workspaceAccess` 不是 `"rw"` 时，工具在 `~/.avaclaw/sandboxes` 下的沙箱工作区内操作，而不是你的主机工作区。
...
`avaclaw onboard`、`avaclaw configure` 或 `avaclaw setup` 将创建工作区并在缺失时填充引导文件。
...
旧版安装可能创建了 `~/avaclaw`。保留多个工作区目录可能会导致混乱的认证或状态漂移，因为同一时间只有一个工作区是活动的。

**建议：** 保持单个活动工作区。如果你不再使用额外的文件夹，请归档或移至废纸篓（例如 `trash ~/avaclaw`）。
如果你有意保留多个工作区，请确保 `agents.defaults.workspace` 指向活动的那个。

`avaclaw doctor` 在检测到额外工作区目录时会发出警告。
...
如果你必须存储敏感引用，请使用占位符并将真正的密钥保存在其他地方（密码管理器、环境变量或 `~/.avaclaw/`）。

...
AvaClaw 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
AvaClaw 支持任何发现域名；`avaclaw.internal.` 只是一个示例。
...
- 在 `~/.avaclaw/avaclaw.json` 中设置 `gateway.bind: "tailnet"`。
...
- `~/.avaclaw/avaclaw.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。

...
`avaclaw doctor` 是 Ava-Claw 的修复 + 迁移工具。它修复过时的配置/状态，检查健康状况，并提供可操作的修复步骤。
...
```bash
avaclaw doctor --repair --force
```

也应用激进的修复（覆盖自定义 supervisor 配置）。

```bash
avaclaw doctor --non-interactive
```
...
当配置包含已弃用的键时，其他命令会拒绝运行并要求你运行 `avaclaw doctor`。

Doctor 将：

- 解释找到了哪些遗留键。
- 显示它应用的迁移。
- 使用更新后的 schema 重写 `~/.avadisabelle/ava-claw.json`。
...
如果你手动添加了 `models.providers.opencode`（或 `opencode-zen`），它会覆盖 `@avadisabelle/ava-pi-ai` 中内置的 OpenCode Zen 目录。这可能会强制将每个模型放到单个 API 上或将成本归零。Doctor 会发出警告，以便你可以移除覆盖并恢复每模型 API 路由 + 成本。
...
- 会话存储 + 记录：
  - 从 `~/.avaclaw/sessions/` 到 `~/.avaclaw/agents/<agentId>/sessions/`
- 智能体目录：
  - 从 `~/.avaclaw/agent/` 到 `~/.avaclaw/agents/<agentId>/agent/`
- WhatsApp 认证状态（Baileys）：
  - 从遗留的 `~/.avaclaw/credentials/*.json`（除 `oauth.json` 外）
  - 到 `~/.avaclaw/credentials/whatsapp/<accountId>/...`（默认账户 id：`default`）

这些迁移是尽力而为且幂等的；当 doctor 将任何遗留文件夹作为备份保留时会发出警告。Gateway 网关/CLI 也会在启动时自动迁移遗留会话 + 智能体目录，因此历史/认证/模型会落在每智能体路径中，无需手动运行 doctor。WhatsApp 认证有意仅通过 `avaclaw doctor` 迁移。
...
- **多个状态目录**：当多个 `~/.avaclaw` 文件夹存在于不同 home 目录或当 `AVACLAW_STATE_DIR` 指向别处时发出警告（历史可能在安装之间分裂）。
- **远程模式提醒**：如果 `gateway.mode=remote`，doctor 会提醒你在远程主机上运行它（状态在那里）。
- **配置文件权限**：当 `~/.avadisabelle/ava-claw.json` 对组/其他用户可读时发出警告，并提供收紧到 `600` 的选项。
...
Doctor 检测遗留的 Gateway 网关服务（launchd/systemd/schtasks），并提供删除它们并使用当前 Gateway 网关端口安装 Ava-Claw 服务的选项。它还可以扫描额外的类 Gateway 网关服务并打印清理提示。
配置文件命名的 Ava-Claw Gateway 网关服务被视为一等公民，不会被标记为"额外的"。
...
当本地 Gateway 网关缺少 `gateway.auth` 时，Doctor 会发出警告并提供生成 token 的选项。使用 `avaclaw doctor --generate-gateway-token` 在自动化中强制创建 token。
...
- `avaclaw doctor` 在重写 supervisor 配置前提示。
- `avaclaw doctor --yes` 接受默认修复提示。
- `avaclaw doctor --repair` 无需提示应用推荐的修复。
- `avaclaw doctor --repair --force` 覆盖自定义 supervisor 配置。
- 你始终可以通过 `avaclaw gateway install --force` 强制完全重写。

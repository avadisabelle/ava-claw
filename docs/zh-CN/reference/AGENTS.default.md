...
read_when:
  - 启动新的 AvaClaw 智能体会话
  - 启用或审计默认 Skills
summary: 个人助手设置的默认 AvaClaw 智能体指令和 Skills 列表
x-i18n:
...
# AGENTS.md — AvaClaw 个人助手（默认）

## 首次运行（推荐）

AvaClaw 为智能体使用专用的工作区目录。默认：`~/.avaclaw/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
- 对于浏览器驱动的验证，使用带有 AvaClaw 管理的 Chrome 配置文件的 `avaclaw browser`（tabs/status/screenshot）。

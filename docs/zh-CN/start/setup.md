...
summary: 设置指南：在保持最新的同时保持你的 Ava-Claw 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.avaclaw/workspace`（工作区）+ `~/.avadisabelle/ava-claw.json`（配置）。
...
- **配置：** `~/.avadisabelle/ava-claw.json`（JSON/JSON5 格式）
- **工作区：** `~/.avaclaw/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm avaclaw setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **Ava-Claw.app**（菜单栏）。
...
- 运行 `avaclaw setup`，然后 `avaclaw channels login`，然后手动启动 Gateway 网关（`avaclaw gateway`）。
...
- **配对允许列表**：`~/.avaclaw/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.avaclaw/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.avaclaw/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.avaclaw/workspace` 和 `~/.avaclaw/` 保持为"你的东西"；不要将个人提示/配置放入 `avaclaw` 仓库。

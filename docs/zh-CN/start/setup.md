...
summary: 设置指南：在保持最新的同时保持你的 AvaClaw 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.avaclaw/workspace`（工作区）+ `~/.avaclaw/avaclaw.json`（配置）。
...
- **配置：** `~/.avaclaw/avaclaw.json`（JSON/JSON5 格式）

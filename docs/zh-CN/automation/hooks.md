...
- **Webhooks**：外部 HTTP webhooks，让其他系统触发 Ava-Claw 中的工作。参见 [Webhook Hooks](/automation/webhook) 或使用 `openclaw webhooks` 获取 Gmail 助手命令。
...
3. **捆绑 hooks**：`<openclaw>/dist/hooks/bundled/`（随 Ava-Claw 附带）

---
summary: "OpenProse: .prose workflows, slash commands, and state in Ava-Claw"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In Ava-Claw it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses Ava-Claw tools under the hood.
...
## Ava-Claw runtime mapping

OpenProse programs map to Ava-Claw primitives:

| OpenProse concept         | Ava-Claw tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use Ava-Claw tool allowlists and approval gates to control side effects.

...
- Provider notes:
  ...
  - Moonshot (`moonshot/*`) maps `/think off` to `thinking: { type: "disabled" }` and any non-`off` level to `thinking: { type: "enabled" }`. When thinking is enabled, Moonshot only accepts `tool_choice` `auto|none`; AvaClaw normalizes incompatible values to `auto`.
...
- AvaClaw resolves fast mode in this order:
...
- For `openai-codex/*`, fast mode applies the same low-latency profile on Codex Responses. AvaClaw keeps one shared `/fast` toggle across both auth paths.
...
- Anthropic fast mode is API-key only. AvaClaw skips Anthropic service-tier injection for Claude setup-token / OAuth auth and for non-Anthropic proxy base URLs.

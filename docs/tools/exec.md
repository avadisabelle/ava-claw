...
- `gateway`/`node` approvals are controlled by `~/.avaclaw/exec-approvals.json`.
...
- Ava-Claw sets `AVACLAW_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Ava-Claw prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`avaclaw security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `avaclaw doctor --fix` can scaffold missing custom `safeBinProfiles` entries.

...
read_when:
  - Running AvaClaw Gateway in WSL2 while Chrome lives on Windows
...
- AvaClaw Gateway runs inside WSL2
...
Use the built-in `chrome-relay` profile plus the AvaClaw Chrome extension.
...
- AvaClaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, AvaClaw is not the problem yet.
...
Fix that before touching AvaClaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point AvaClaw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting AvaClaw to succeed
...
3. AvaClaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug AvaClaw config or Control UI auth

...
read_when:
  - Running Ava-Claw Gateway in WSL2 while Chrome lives on Windows
...
- Ava-Claw Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/avadisabelle/ava-claw/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome-relay` profile plus the Ava-Claw Chrome extension.
...
- Ava-Claw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Ava-Claw is not the problem yet.
...
Fix that before touching Ava-Claw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Ava-Claw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Ava-Claw to succeed
...
3. Ava-Claw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Ava-Claw config or Control UI auth

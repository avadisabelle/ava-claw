...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `avaclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `avaclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=avaclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${AVACLAW_STATE_DIR:-$HOME/.avaclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```

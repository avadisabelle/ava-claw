...
- **gateway host** → `avaclaw` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.avaclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.avaclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `avaclaw approvals allowlist ...`).
- `avaclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `avaclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.avaclaw/exec-approvals.json` directly.

CLI: `avaclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).

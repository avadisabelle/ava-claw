...
summary: "Architecture of AvaClaw's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in AvaClaw
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how AvaClaw integrates with [pi-coding-agent](https://github.com/avadisabelle/ava-pi/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

AvaClaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, AvaClaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
```json
{
  "@avadisabelle/ava-pi-agent-core": "0.49.3",
  "@avadisabelle/ava-pi-ai": "0.49.3",
  "@avadisabelle/ava-pi-coding-agent": "0.49.3",
  "@avadisabelle/ava-pi-tui": "0.49.3"
}
```
...
| `pi-tui`          | Terminal UI components (used in AvaClaw's local TUI mode)                                              |
...
├── pi-tools.ts                    # createAvaClawCodingTools()
...
├── avaclaw-tools.ts              # AvaClaw-specific tools
...
Image injection is prompt-local: AvaClaw loads image refs from the current prompt and
...
2. **Custom Replacements**: AvaClaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **AvaClaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures AvaClaw's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, AvaClaw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
AvaClaw wraps this with `guardSessionManager()` for tool result safety.
...
AvaClaw maintains an auth profile store with multiple API keys per provider:
...
AvaClaw loads custom pi extensions for specialized behavior:
...
AvaClaw also has a local TUI mode that uses pi-tui components directly:

```typescript
// src/tui/tui.ts
import { ... } from "@avadisabelle/ava-pi-tui";
```
...
| Aspect          | Pi CLI                  | AvaClaw Embedded                                                                             |
| --------------- | ----------------------- | -------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                               |
| Tools           | Default coding tools    | Custom AvaClaw tool suite                                                                    |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `AVACLAW_LIVE_TEST=1`)

...
summary: "Architecture of Ava-Claw's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Ava-Claw
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Ava-Claw integrates with [pi-coding-agent](https://github.com/avadisabelle/ava-pi/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Ava-Claw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Ava-Claw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Ava-Claw's local TUI mode)                                             |
...
├── pi-tools.ts                    # createAva-ClawCodingTools()
...
├── avaclaw-tools.ts              # Ava-Claw-specific tools
...
Image injection is prompt-local: Ava-Claw loads image refs from the current prompt and
...
2. **Custom Replacements**: Ava-Claw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Ava-Claw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Ava-Claw's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Ava-Claw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Ava-Claw wraps this with `guardSessionManager()` for tool result safety.
...
Ava-Claw maintains an auth profile store with multiple API keys per provider:
...
Ava-Claw loads custom pi extensions for specialized behavior:
...
Ava-Claw also has a local TUI mode that uses pi-tui components directly:

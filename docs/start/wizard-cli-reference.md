...
read_when:
  - You need detailed behavior for avaclaw onboard
...
This page is the full reference for `avaclaw onboard`.
...
<Steps>
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `avaclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `avaclaw health`.
    - `avaclaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.avaclaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.avaclaw/agents/<agentId>/agent/auth-profiles.json`
...
`~/.avaclaw/credentials/oauth.json` (or `$AVACLAW_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`avaclaw onboard`](/cli/onboard)

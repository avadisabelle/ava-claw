...
```bash
# Set hostname
sudo hostnamectl set-hostname avaclaw
...
sudo tailscale up --ssh --hostname=avaclaw
```

This enables Tailscale SSH, so you can connect via `ssh avaclaw` from any device on your tailnet — no public IP needed.
...
avaclaw config set gateway.bind loopback

# Require auth for the Gateway + Control UI
avaclaw config set gateway.auth.mode token
avaclaw doctor --generate-gateway-token

# Expose over Tailscale Serve (HTTPS + tailnet access)
avaclaw config set gateway.tailscale.mode serve
avaclaw config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart avaclaw-gateway
```
...
# Check daemon status
systemctl --user status avaclaw-gateway
...
```
https://avaclaw.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `avaclaw security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.avaclaw`
- **Security audit:** `avaclaw security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@avaclaw
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=avaclaw --reset
```
...
journalctl --user -u avaclaw-gateway -n 50
```
...
- `~/.avaclaw/` — config, credentials, session data
- `~/.avaclaw/workspace/` — workspace (SOUL.md, memory, artifacts)

Back up periodically:

```bash
tar -czvf avaclaw-backup.tar.gz ~/.avaclaw ~/.avaclaw/workspace
```

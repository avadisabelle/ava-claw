...
service via the external `avaclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.avaclaw.gateway`
  (or `ai.avaclaw.<profile>` when using `--profile`/`AVACLAW_PROFILE`; legacy `com.avaclaw.*` is supported).

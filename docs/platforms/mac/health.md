...
- You can still use the CLI flow in [Gateway health](/gateway/health) (`openclaw status`, `openclaw status --deep`, `openclaw health --json`) and tail `/tmp/avadisabelle/ava-claw-*.log` for `web-heartbeat` / `web-reconnect`.

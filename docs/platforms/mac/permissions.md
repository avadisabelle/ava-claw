...
- Same path: run the app from a fixed location (for Ava-Claw, `dist/Ava-Claw.app`).
...
```bash
sudo tccutil reset Accessibility ai.avaclaw.mac
sudo tccutil reset ScreenCapture ai.avaclaw.mac
...
Workaround: move files into the Ava-Claw workspace (`~/.avaclaw/workspace`) if you want to avoid per-folder grants.

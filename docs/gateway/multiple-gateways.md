---
summary: "Run multiple Ava-Claw Gateways on one host (isolation, ports, and profiles)"
read_when:
...
- `AVACLAW_CONFIG_PATH` — per-instance config file
- `AVACLAW_STATE_DIR` — per-instance sessions, creds, caches
...
Profiles auto-scope `AVACLAW_STATE_DIR` + `AVACLAW_CONFIG_PATH` and suffix service names.
...
Base port = `gateway.port` (or `AVACLAW_GATEWAY_PORT` / `--port`).

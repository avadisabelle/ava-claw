---
summary: "How Ava-Claw sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Ava-Claw can run **tools inside Docker containers** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`avaclaw-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; Ava-Claw emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
- `"none"` (default): tools see a sandbox workspace under `~/.avaclaw/sandboxes`.
- `"ro"`: mounts the agent workspace read-only at `/agent` (disables `write`/`edit`/`apply_patch`).
- `"rw"`: mounts the agent workspace read/write at `/workspace`.

Inbound media is copied into the active sandbox workspace (`media/inbound/*`).
Skills note: the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`,
Ava-Claw mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- Ava-Claw blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
Default image: `avaclaw-sandbox:bookworm-slim`
...
`avaclaw-sandbox-common:bookworm-slim`.
...
- The three graphics hardening flags (`--disable-3d-apis`,
  ...
  when containers lack GPU support. Set `AVACLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0`
...
- `--disable-extensions` is enabled by default and can be disabled with
  `AVACLAW_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
- `--renderer-process-limit=2` is controlled by
  `AVACLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
Set `AVACLAW_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can
override socket location with `AVACLAW_DOCKER_SOCKET`. Full setup and env
...
- Use `avaclaw sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.

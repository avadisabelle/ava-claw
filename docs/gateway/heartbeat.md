...
- During heartbeat runs, AvaClaw treats `HEARTBEAT_OK` as an ack when it appears
...
If **all three** are false, AvaClaw skips the heartbeat run entirely (no model call).

...
- During heartbeat runs, Ava-Claw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `avaclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Ava-Claw skips the heartbeat run entirely (no model call).
...
```bash
avaclaw system event --text "Check for urgent follow-ups" --mode now
```

...
## Quick check: `avaclaw security audit`
...
4. Claim is not listed in [Out of Scope](https://github.com/avadisabelle/ava-claw/blob/main/SECURITY.md#out-of-scope).
...
`avaclaw security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`avaclaw security audit` includes `config.insecure_or_dangerous_flags` when
...
- “Paste the full contents of ~/.avaclaw or your logs.”
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `AVACLAW_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__avaclaw__/canvas/` and `/__avaclaw__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_avaclaw-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `AVACLAW_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `avaclaw doctor --generate-gateway-token`.
...
paths, set `AVACLAW_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `AVACLAW_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `AVACLAW_GATEWAY_PASSWORD`).
...
Assume anything under `~/.avaclaw/` (or `$AVACLAW_STATE_DIR/`) may contain secrets or private data:

- `avaclaw.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `avaclaw status --all` (pasteable, secrets redacted) over raw logs.
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.avaclaw`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.avaclaw/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `avaclaw` profile).
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `avaclaw gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `AVACLAW_GATEWAY_PASSWORD`) and restart.

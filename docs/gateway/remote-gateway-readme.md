...
```bash
launchctl setenv AVACLAW_GATEWAY_TOKEN "<your-token>"
```
...
Save this as `~/Library/LaunchAgents/ai.avaclaw.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.avaclaw.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.avaclaw.ssh-tunnel.plist
```
...
Legacy note: remove any leftover `com.avaclaw.ssh-tunnel` LaunchAgent if present.
...
```bash
launchctl kickstart -k gui/$UID/ai.avaclaw.ssh-tunnel
```

**Stop the tunnel:**

```bash
launchctl bootout gui/$UID/ai.avaclaw.ssh-tunnel
```

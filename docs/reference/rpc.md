...
AvaClaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- AvaClaw owns lifecycle when `channels.signal.autoStart=true`.
...
- AvaClaw spawns `imsg rpc` as a child process (legacy iMessage integration).

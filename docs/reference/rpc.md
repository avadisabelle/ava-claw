...
Ava-Claw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Ava-Claw owns lifecycle when `channels.signal.autoStart=true`.
...
- Ava-Claw spawns `imsg rpc` as a child process (legacy iMessage integration).

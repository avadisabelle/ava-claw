...
This document explains how Ava-Claw manages sessions end-to-end:
...
Ava-Claw is designed around a single **Gateway process** that owns session state.
...
Ava-Claw persists sessions in two layers:
...
In `mode: "warn"`, Ava-Claw reports potential evictions but does not mutate the store/files.
...
Ava-Claw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Ava-Claw consumes the events, but Pi decides when to compact).
...
Ava-Claw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Ava-Claw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Ava-Claw leaves it alone.
...
Ava-Claw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Ava-Claw strips/suppresses this in the delivery layer.

As of `2026.1.10`, Ava-Claw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Ava-Claw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Ava-Claw’s

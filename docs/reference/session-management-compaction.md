...
This document explains how AvaClaw manages sessions end-to-end:
...
AvaClaw is designed around a single **Gateway process** that owns session state.
...
AvaClaw persists sessions in two layers:
...
AvaClaw resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, AvaClaw reports potential evictions but does not mutate the store/files.
...
Transcripts are managed by `@avadisabelle/ava-pi-coding-agent`’s `SessionManager`.
...
AvaClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (AvaClaw consumes the events, but Pi decides when to compact).
...
AvaClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, AvaClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, AvaClaw leaves it alone.
...
AvaClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- AvaClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, AvaClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
AvaClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but AvaClaw’s

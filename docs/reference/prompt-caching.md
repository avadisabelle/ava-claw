...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.avaclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$AVACLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `AVACLAW_CACHE_TRACE=1` enables cache tracing.
- `AVACLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `AVACLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `AVACLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `AVACLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.

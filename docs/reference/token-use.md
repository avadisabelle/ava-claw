---
summary: "How Ava-Claw builds prompt context and reports token usage + costs"
read_when:
...
Ava-Claw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Ava-Claw assembles its own system prompt on every run. It includes:
...
For images, Ava-Claw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Ava-Claw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `avaclaw status --usage` and `avaclaw channels list` show
...
`cacheWrite`. If pricing is missing, Ava-Claw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Ava-Claw can
...
Anthropic's 1M context window is currently beta-gated. Ava-Claw can inject the
...
Ava-Claw skips the `context-1m-*` beta header because Anthropic currently

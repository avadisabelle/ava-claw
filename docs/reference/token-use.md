---
summary: "How AvaClaw builds prompt context and reports token usage + costs"
read_when:
...
AvaClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

AvaClaw assembles its own system prompt on every run. It includes:
...
For images, AvaClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from AvaClaw session logs.
...
`cacheWrite`. If pricing is missing, AvaClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. AvaClaw can
...
Anthropic's 1M context window is currently beta-gated. AvaClaw can inject the
...
AvaClaw skips the `context-1m-*` beta header because Anthropic currently

...
This is the full reference for the `avaclaw onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.avaclaw/credentials/oauth.json`; auth profiles live in `~/.avaclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.avaclaw/credentials/oauth.json` (or `$AVACLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.avaclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `avaclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `avaclaw configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `avaclaw health`.
    - Tip: `avaclaw status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export AVACLAW_GATEWAY_TOKEN="your-token"
avaclaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env AVACLAW_GATEWAY_TOKEN
```
...
- Stores it under `~/.avaclaw/tools/signal-cli/<version>/`.
...
`avaclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.avaclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.avaclaw/agents/<agentId>/sessions/`.

...
```bash
avaclaw plugins install @avadisabelle/ava-claw-nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
avaclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Ava-Claw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `avaclaw pairing list nextcloud-talk`
  - `avaclaw pairing approve nextcloud-talk <CODE>`

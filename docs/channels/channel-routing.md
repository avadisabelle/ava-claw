...
Ava-Claw routes replies **back to the channel where a message came from**. The
...
Ava-Claw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Ava-Claw still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Ava-Claw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.avaclaw/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.avaclaw`):

- `~/.avaclaw/agents/<agentId>/sessions/sessions.json`

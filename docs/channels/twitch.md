...
read_when:
  - Setting up Twitch chat integration for AvaClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. AvaClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `AVACLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "avaclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use AVACLAW_TWITCH_ACCESS_TOKEN env var)

...
Matrix is an open, decentralized messaging protocol. AvaClaw connects as a Matrix **user**
...
```bash
avaclaw plugins install ava-claw-matrix
```
...
1. Install the Matrix plugin:
   - From npm: `avaclaw plugins install ava-claw-matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: AvaClaw calls the same
...
- On first connection, AvaClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  AvaClaw logs a warning.
...
- On startup, AvaClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.

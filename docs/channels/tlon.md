...
Tlon is a decentralized messenger built on Urbit. AvaClaw connects to your Urbit ship and can
...
```bash
avaclaw plugins install ava-claw-tlon
```
...
By default, AvaClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
- Thread replies: if the inbound message is in a thread, AvaClaw replies in-thread.

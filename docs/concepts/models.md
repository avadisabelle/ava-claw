...
AvaClaw selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models AvaClaw can use (plus aliases).
...
Marker persistence is source-authoritative: AvaClaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever AvaClaw regenerates `models.json`, including command-driven paths like `avaclaw agent`.

...
- `/usage cost` in chats: local cost summary aggregated from Ava-Claw session logs.
- CLI: `avaclaw status --usage` prints a full per-provider breakdown.
- CLI: `avaclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).

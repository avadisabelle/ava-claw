---
title: "Memory"
summary: "How Ava-Claw memory works (workspace files + automatic memory flush)"
read_when:
...
Ava-Claw memory is **plain Markdown in the agent workspace**. The files are the
...
- `MEMORY.md` (optional)
  ...
  - If both `MEMORY.md` and `memory.md` exist at the workspace root, Ava-Claw only loads `MEMORY.md`.
  - Lowercase `memory.md` is only used as a fallback when `MEMORY.md` is absent.
  - **Only load in the main, private session** (never in group contexts).

These files live under the workspace (`agents.defaults.workspace`, default
`~/.avaclaw/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

Ava-Claw exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, Ava-Claw triggers a **silent,
...
Ava-Claw can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, Ava-Claw auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. Ava-Claw
...
BM25 + vectors + reranking. Markdown stays the source of truth; Ava-Claw shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.avaclaw/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.avaclaw/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, Ava-Claw retries with `qmd query`. If QMD fails or the binary is
  missing, Ava-Claw automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- Ava-Claw does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - Ava-Claw sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index Ava-Claw
    uses), run a one-off query with the agent’s XDG dirs.

    Ava-Claw’s QMD state lives under your **state dir** (defaults to `~/.avaclaw`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    Ava-Claw uses:

    ```bash
    # Pick the same state dir Ava-Claw uses
    STATE_DIR="${AVACLAW_STATE_DIR:-$HOME/.avaclaw}"
...
- When `scope` denies a search, Ava-Claw logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, Ava-Claw exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/.avaclaw/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- If `memorySearch.multimodal.enabled = true`, Ava-Claw also indexes supported image/audio files under `extraPaths` only. Default memory roots (`MEMORY.md`, `memory.md`, `memory/**/*.md`) stay Markdown-only.
- Symlinks are ignored (files or directories).

### Multimodal memory files (Gemini image + audio)

Ava-Claw can index image and audio files from `memorySearch.extraPaths` when using Gemini embedding 2:
...
> Ava-Claw will automatically reindex when it detects a model or dimension change.
...
- Index storage: per-agent SQLite at `~/.avaclaw/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
- Freshness: watcher on `MEMORY.md` + `memory/` marks the index dirty (debounce 1.5s). Sync is scheduled on session start, on search, or on an interval and runs asynchronously. Session transcripts use delta thresholds to trigger background sync.
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, Ava-Claw automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, Ava-Claw combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, Ava-Claw falls back to vector-only search.
...
Ava-Claw can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/.avaclaw/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available, Ava-Claw stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, Ava-Claw logs the

---
summary: "VPS hosting hub for Ava-Claw (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/avaclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `AVACLAW_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.

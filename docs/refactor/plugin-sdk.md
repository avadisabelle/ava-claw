...
Accessed via `AvaClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AvaClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AvaClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AvaClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AvaClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AvaClawConfig): string;
  };
};
```
...
- Add `api.runtime` to `AvaClawPluginApi` with the surface above.

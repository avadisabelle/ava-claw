...
- Publish as `avaclaw/plugin-sdk` (or export from core under `avaclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `Ava-ClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: Ava-ClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: Ava-ClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: Ava-ClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: Ava-ClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: Ava-ClawConfig): string;
  };
};
```
...
- Introduce `avaclaw/plugin-sdk`.
- Add `api.runtime` to `Ava-ClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `avaclawRuntime: ">=2026.2.0"`).

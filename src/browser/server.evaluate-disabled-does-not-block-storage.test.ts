  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          avaclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.AVACLAW_GATEWAY_PORT;
    process.env.AVACLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.AVACLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.AVACLAW_GATEWAY_PASSWORD;

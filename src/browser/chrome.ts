...
export function resolveAvaClawUserDataDir(profileName = DEFAULT_AVACLAW_BROWSER_PROFILE_NAME) {
...
export async function launchAvaClawChrome(
  ...
  const userDataDir = resolveAvaClawUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_AVACLAW_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateAvaClawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 avaclaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`avaclaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`avaclaw browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 avaclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopAvaClawChrome(

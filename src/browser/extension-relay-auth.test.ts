...
import {
  probeAuthenticatedAvaClawRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedAvaClawRelay({
    baseUrl,
    relayAuthHeader: "x-avaclaw-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated avaclaw relay probe responses", async () => {

...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".avaclaw-profile-decorated");
}
 ...
 */
export function decorateAvaClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_AVACLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_AVACLAW_BROWSER_COLOR).toUpperCase();

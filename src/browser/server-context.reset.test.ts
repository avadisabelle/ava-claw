...
function localAvaClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "avaclaw",
...
function createLocalAvaClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localAvaClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveAvaClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "avaclaw-reset-"));
    const profileDir = path.join(tempRoot, "avaclaw");
    ...
    const ops = createLocalAvaClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveAvaClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "avaclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "avaclaw");
    fs.mkdirSync(profileDir, { recursive: true });

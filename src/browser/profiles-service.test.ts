...
vi.mock("./chrome.js", () => ({
  resolveAvaClawUserDataDir: vi.fn(() => "/tmp/avaclaw-test/avaclaw/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "avaclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveAvaClawUserDataDir).mockReturnValue(userDataDir);

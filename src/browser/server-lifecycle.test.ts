...
const { stopAvaClawChromeMock, stopChromeExtensionRelayServerMock } = vi.hoisted(() => ({
  stopAvaClawChromeMock: vi.fn(async () => {}),
...
vi.mock("./chrome.js", () => ({
  stopAvaClawChrome: stopAvaClawChromeMock,
}));
...
describe("ensureExtensionRelayForProfiles", () => {
  ...
  it("starts relay only for extension profiles", async () => {
    resolveProfileMock.mockImplementation((_resolved: unknown, name: string) => {
      ...
      return { driver: "avaclaw", cdpUrl: "http://127.0.0.1:18889" };
    });
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopAvaClawChromeMock.mockClear();
    stopChromeExtensionRelayServerMock.mockClear();
  });

  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["avaclaw", "chrome-relay"]);
    ...
    expect(stopMap.avaclaw).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopAvaClawChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("avaclaw browser stop failed: Error: oops");
  });
});

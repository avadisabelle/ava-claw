...
import type { ConfigFileSnapshot } from "./types.avaclaw.js";
import { AvaClawSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(AvaClawSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.avaclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/avaclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/avaclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/avaclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/avaclaw/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = AvaClawSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "AvaClawConfig";
    const hints = mainSchemaHints;

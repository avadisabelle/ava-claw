...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ AVACLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-avaclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-avaclaw#general");
  });
  ...
  it("derives session transcripts dir from AVACLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { AVACLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when AVACLAW_STATE_DIR differs from stored paths", () => {

...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("avaclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "avaclaw-sandbox-browser-global" },
      agentBrowser: { network: "avaclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("avaclaw-sandbox-browser-agent");
  });

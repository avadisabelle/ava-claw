...
function resolveRepoRoot(): string {
  const fromPackage = resolveAvaClawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    AVACLAW_STATE_DIR: path.join(os.tmpdir(), "avaclaw-config-doc-baseline-state"),
    AVACLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };

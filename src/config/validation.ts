...
import type { AvaClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AvaClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: AvaClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: AvaClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AvaClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AvaClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AvaClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as AvaClawConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as AvaClawConfig,
  };
}
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native AvaClaw config schema.

  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.avaclaw/logs/avaclaw.log" },
        hooks: {
          path: "~/.avaclaw/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.avaclaw/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".avaclaw", "logs", "avaclaw.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".avaclaw", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".avaclaw", "telegram.token"),
      );

...
import type { AvaClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareAvaClawVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  avaclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  avaclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: AvaClawConfig,
  pathSegments: string[],
): { changed: boolean; next: AvaClawConfig } {
...
function coerceConfig(value: unknown): AvaClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AvaClawConfig;
}
...
function stampConfigVersion(cfg: AvaClawConfig): AvaClawConfig {
...
function warnIfConfigFromFuture(cfg: AvaClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareAvaClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer AvaClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as AvaClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AvaClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AvaClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AvaClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as AvaClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.AVACLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.AVACLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.AVACLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.AVACLAW_WATCH_SESSION === "string" &&
        deps.env.AVACLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.AVACLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.AVACLAW_WATCH_COMMAND === "string" &&
        deps.env.AVACLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.AVACLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `AVACLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: AvaClawConfig | null = null;
let runtimeConfigSourceSnapshot: AvaClawConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AVACLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.AVACLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): AvaClawConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): AvaClawConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: AvaClawConfig;
  candidate: AvaClawConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: AvaClawConfig): AvaClawConfig {
...
export function loadConfig(): AvaClawConfig {
...
export async function readBestEffortConfig(): Promise<AvaClawConfig> {
...
export async function writeConfigFile(
  ...
  // edits to avaclaw.json remain visible (no stale snapshot).
}

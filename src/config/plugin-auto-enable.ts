...
function isWhatsAppConfigured(cfg: AvaClawConfig): boolean {
...
function isGenericChannelConfigured(cfg: AvaClawConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: AvaClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AvaClawConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: AvaClawConfig, env: NodeJS.ProcessEnv): string[] {
...
function isPluginExplicitlyDisabled(cfg: AvaClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: AvaClawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: AvaClawConfig, pluginId: string): AvaClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;

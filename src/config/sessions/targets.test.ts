...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): AvaClawConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: AvaClawConfig, env: NodeJS.ProcessEnv) =>
    ...
    expect(targets).toEqual([
      {
        ...
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".avaclaw/agents/main/sessions/sessions.json"),
        ),
      },
      {
        agentId: "work",
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".avaclaw/agents/work/sessions/sessions.json"),
        ),
      },
    ]);
  });

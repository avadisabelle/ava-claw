...
import { buildWebSearchProviderConfig, withTempHome, writeAvaClawConfig } from "./test-helpers.js";
import { AvaClawSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = AvaClawSchema.safeParse({
      $schema: "https://avaclaw.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://avaclaw.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = AvaClawSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = AvaClawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeAvaClawConfig(home, {
        gateway: { bind: "${AVACLAW_BIND}" },
      });

      const prev = process.env.AVACLAW_BIND;
      process.env.AVACLAW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.AVACLAW_BIND;
        } else {
          process.env.AVACLAW_BIND = prev;
        }
      }
    });
  });

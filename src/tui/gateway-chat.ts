...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set AVACLAW_GATEWAY_TOKEN/AVACLAW_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.AVACLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.AVACLAW_GATEWAY_PASSWORD);

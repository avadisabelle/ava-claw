...
describe("resolveInitialTuiAgentId", () => {
  const cfg: AvaClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/avaclaw" },
        { id: "ops", workspace: "/tmp/avaclaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/avaclaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run avaclaw devices list");
    expect(state.pairingHint).toContain("avaclaw devices list");
  });

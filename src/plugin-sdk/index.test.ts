...
const pluginSdkSpecifiers = pluginSdkEntrypoints.map((entry) =>
  entry === "index" ? "avaclaw/plugin-sdk" : `avaclaw/plugin-sdk/${entry}`,
);
...
describe("plugin-sdk exports", () => {
  ...
  it("emits importable bundled subpath entries", { timeout: 240_000 }, async () => {
    const outDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-plugin-sdk-build-"));
    const fixtureDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-plugin-sdk-consumer-"));
    ...
    try {
      ...
      const packageDir = path.join(fixtureDir, "avaclaw");
      ...
      await fs.symlink(packageDir, path.join(consumerDir, "node_modules", "avaclaw"), "dir");
      await fs.writeFile(
        consumerEntry,
        [

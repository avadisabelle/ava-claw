...
let originalAvaClawLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.AVACLAW_TEST_FILE_LOG;
  originalAvaClawLogLevel = process.env.AVACLAW_LOG_LEVEL;
  delete process.env.AVACLAW_TEST_FILE_LOG;
  delete process.env.AVACLAW_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.AVACLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalAvaClawLogLevel === undefined) {
    delete process.env.AVACLAW_LOG_LEVEL;
  } else {
    process.env.AVACLAW_LOG_LEVEL = originalAvaClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();

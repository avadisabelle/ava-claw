...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.AVACLAW_GATEWAY_PORT).toBe("18789");
    expect(env.AVACLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.AVACLAW_SERVICE_MARKER).toBe("avaclaw");
    expect(env.AVACLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.AVACLAW_SERVICE_VERSION).toBe("string");
    expect(env.AVACLAW_SYSTEMD_UNIT).toBe("avaclaw-gateway.service");
    expect(env.AVACLAW_WINDOWS_TASK_NAME).toBe("AvaClaw Gateway");
    if (process.platform === "darwin") {
      expect(env.AVACLAW_LAUNCHD_LABEL).toBe("ai.avaclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", AVACLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.AVACLAW_SYSTEMD_UNIT).toBe("avaclaw-gateway-work.service");
    expect(env.AVACLAW_WINDOWS_TASK_NAME).toBe("AvaClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.AVACLAW_LAUNCHD_LABEL).toBe("ai.avaclaw.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through AVACLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", AVACLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.AVACLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to AVACLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.AVACLAW_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers AVACLAW_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        AVACLAW_GATEWAY_TOKEN: "avaclaw-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.AVACLAW_GATEWAY_TOKEN).toBe("avaclaw-token");
  });

  it("omits AVACLAW_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".avaclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", AVACLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".avaclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", AVACLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".avaclaw"));
  });

  it("uses AVACLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", AVACLAW_STATE_DIR: "/var/lib/avaclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/avaclaw"));
  });

  it("expands ~ in AVACLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", AVACLAW_STATE_DIR: "~/avaclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/avaclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { AVACLAW_STATE_DIR: "C:\\State\\avaclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\avaclaw");
  });
});

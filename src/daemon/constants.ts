...
export function resolveGatewayWindowsTaskName(profile?: string): string {
  ...
  return `AvaClaw Gateway (${normalized})`;
}
...
export function formatGatewayServiceDescription(params?: {
  ...
  if (parts.length === 0) {
    return "AvaClaw Gateway";
  }
  return `AvaClaw Gateway (${parts.join(", ")})`;
}
...
export function resolveGatewayServiceDescription(params: {
  ...
  return (
    ...
    formatGatewayServiceDescription({
      profile: params.env.AVACLAW_PROFILE,
      version: params.environment?.AVACLAW_SERVICE_VERSION ?? params.env.AVACLAW_SERVICE_VERSION,
    })
  );
}
...
export function formatNodeServiceDescription(params?: { version?: string }): string {
  const version = params?.version?.trim();
  if (!version) {
    return "AvaClaw Node Host";
  }
  return `AvaClaw Node Host (v${version})`;
}

...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.AVACLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    AVACLAW_PROFILE: profile,
    AVACLAW_GATEWAY_PORT: String(port),
    AVACLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    AVACLAW_SYSTEMD_UNIT: systemdUnit,
    AVACLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    AVACLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    AVACLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.AVACLAW_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    AVACLAW_GATEWAY_TOKEN: gatewayToken,
    AVACLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    AVACLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    AVACLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    AVACLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    AVACLAW_LOG_PREFIX: "node",
    AVACLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    AVACLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    AVACLAW_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    AVACLAW_STATE_DIR: sharedEnv.stateDir,
    AVACLAW_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.AVACLAW_STATE_DIR;
  const configPath = env.AVACLAW_CONFIG_PATH;

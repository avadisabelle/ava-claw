...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `avaclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      AVACLAW_LAUNCHD_LABEL: `ai.avaclaw.launchd-int-${testId}`,
      AVACLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });

...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds AVACLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `avaclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service AVACLAW_GATEWAY_TOKEN does not match gateway.auth.token in avaclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.AVACLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.AVACLAW_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `avaclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }

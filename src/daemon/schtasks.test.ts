...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when AVACLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".avaclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when AVACLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", AVACLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".avaclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers AVACLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        AVACLAW_PROFILE: "rescue",
        AVACLAW_STATE_DIR: "C:\\State\\avaclaw",
      },
      expected: path.join("C:\\State\\avaclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", AVACLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".avaclaw", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem AvaClaw Gateway",
          "cd /d C:\\Projects\\avaclaw",
          "set NODE_ENV=production",
          "set AVACLAW_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\avaclaw",
          environment: {
            NODE_ENV: "production",
            AVACLAW_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\avaclaw\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\avaclaw\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\AvaClaw Share\\node.exe" "\\\\fileserver\\AvaClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\AvaClaw Share\\node.exe",
            "\\\\fileserver\\AvaClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from AVACLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ AVACLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),

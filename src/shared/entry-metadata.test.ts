...
describe("shared/entry-metadata", () => {
  it("prefers metadata emoji and homepage when present", () => {
    expect(
      resolveEmojiAndHomepage({
        metadata: { emoji: "🦀", homepage: " https://avaclaw.ai " },
  ...
  it("falls back through frontmatter homepage aliases and drops blanks", () => {
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { emoji: "🙂", website: " https://docs.avaclaw.ai " },
      }),
    ).toEqual({
      emoji: "🙂",
    ...
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { url: " https://avaclaw.ai/install " },
      }),
    ).toEqual({
      homepage: "https://avaclaw.ai/install",
    });
  });

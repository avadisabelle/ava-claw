...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    expect(coerceIdentityValue("  AvaClaw  ", 20)).toBe("AvaClaw");
    expect(coerceIdentityValue("  AvaClaw  ", 8)).toBe("AvaClaw");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  AvaClaw Assistant  ", 8)).toBe("AvaClaw");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  AvaClaw  ", 0)).toBe("");
    expect(coerceIdentityValue("  AvaClaw  ", -1)).toBe("OpenCla");
  });
});

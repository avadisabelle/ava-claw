...
function buildCoreToolGroupMap() {
  ...
  const avaclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInAvaClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:avaclaw": avaclawTools,

...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.AVACLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".avaclaw", `workspace-${profile}`);
  }
  return path.join(home, ".avaclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".avaclaw";

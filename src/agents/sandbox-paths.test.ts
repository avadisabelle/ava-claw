...
import { resolvePreferredAvaClawTmpDir } from "../infra/tmp-avaclaw-dir.js";
...
async function withOutsideHardlinkInAvaClawTmp<T>(
  params: {
    avaClawTmpDir: string;
  ...
  const hardlinkPath = path.join(params.avaClawTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.avaClawTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.avaClawTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.avaClawTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const avaClawTmpDir = resolvePreferredAvaClawTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred AvaClaw tmp root",
      media: path.join(avaClawTmpDir, "image.png"),
      expected: path.join(avaClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred AvaClaw tmp root",
      media: pathToFileURL(path.join(avaClawTmpDir, "photo.png")).href,
      expected: path.join(avaClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred AvaClaw tmp root",
      media: path.join(avaClawTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(avaClawTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(avaClawTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside avaclaw tmp root",
      media: path.join(os.tmpdir(), "outside-avaclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked AvaClaw tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(avaClawTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(avaClawTmpDir, { recursive: true });
      const symlinkPath = path.join(avaClawTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked AvaClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAvaClawTmp(
      {
        avaClawTmpDir,
  ...
  it("rejects symlinked AvaClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAvaClawTmp(

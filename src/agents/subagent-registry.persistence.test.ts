...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["AVACLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-subagent-"));
  ...
  it("uses isolated temp state when AVACLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "avaclaw-test-state"));
  });
});

import type { Skill } from "@avadisabelle/ava-pi-coding-agent";
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseAvaClawManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyAvaClawManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveAvaClawMetadata(
  frontmatter: ParsedSkillFrontmatter,
): AvaClawSkillMetadata | undefined {
  const metadataObj = resolveAvaClawManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveAvaClawManifestRequires(metadataObj);
  const install = resolveAvaClawManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveAvaClawManifestOs(metadataObj);
  return {

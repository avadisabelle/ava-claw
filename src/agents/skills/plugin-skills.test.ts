    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "avaclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "avaclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "avaclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("avaclaw-");
  const acpxRoot = await tempDirs.make("avaclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("avaclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("avaclaw-");
  const pluginRoot = await tempDirs.make("avaclaw-plugin-");
  const outsideDir = await tempDirs.make("avaclaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("avaclaw-");
    const pluginRoot = await tempDirs.make("avaclaw-claude-bundle-");

import { codingTools, createReadTool, readTool } from "@avadisabelle/ava-pi-coding-agent";
...
function applyModelProviderToolPolicy(
  ...
  // xAI/Grok providers expose a native web_search tool; sending AvaClaw's
...
function resolveExecConfig(params: { cfg?: AvaClawConfig; agentId?: string }) {
...
export function createAvaClawCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createAvaClawReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });

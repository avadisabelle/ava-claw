...
function mockContextDeps(params: {
  ...
  vi.doMock("./models-config.js", () => ({
    ensureAvaClawModelsJson: vi.fn(async () => {}),
  }));
  vi.doMock("./agent-paths.js", () => ({
    resolveAvaClawAgentDir: () => "/tmp/avaclaw-agent",
  }));
...
describe("lookupContextTokens", () => {
  ...
  it("does not skip eager warmup when --profile is followed by -- terminator", async () => {
    ...
    process.argv = ["node", "avaclaw", "--profile", "--", "config", "validate"];
    try {
  ...
  it("skips eager warmup for logs commands that do not need model metadata at startup", async () => {
    ...
    const argvSnapshot = process.argv;
    process.argv = ["node", "avaclaw", "logs", "--limit", "5"];
    try {
      await import("./context.js");
  ...
  it("retries config loading after backoff when an initial load fails", async () => {
    ...
    const argvSnapshot = process.argv;
    process.argv = ["node", "avaclaw", "config", "validate"];
    try {
      const { lookupContextTokens } = await import("./context.js");

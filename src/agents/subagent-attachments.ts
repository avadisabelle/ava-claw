...
function resolveAttachmentLimits(config: AvaClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".avaclaw", "attachments");
  const relDir = path.posix.join(".avaclaw", "attachments", attachmentId);

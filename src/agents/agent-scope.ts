...
type AgentEntry = NonNullable<NonNullable<AvaClawConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: AvaClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: AvaClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: AvaClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: AvaClawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: AvaClawConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: AvaClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: AvaClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);

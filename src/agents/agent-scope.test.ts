...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: AvaClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/avaclaw" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: AvaClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: AvaClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: AvaClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: AvaClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: AvaClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: AvaClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: AvaClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/avaclaw");
  });

  it("uses AVACLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "avaclaw-home");
    vi.stubEnv("AVACLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as AvaClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".avaclaw", "workspace"));
  });

  it("uses AVACLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "avaclaw-home");
    vi.stubEnv("AVACLAW_HOME", home);
    // Clear state dir so it falls back to AVACLAW_HOME
    vi.stubEnv("AVACLAW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as AvaClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".avaclaw", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/avaclaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/avaclaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "avaclaw-agent-scope-"));

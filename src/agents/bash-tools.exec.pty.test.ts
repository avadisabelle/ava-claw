...
test("exec sets AVACLAW_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-avaclaw-shell", {
    command: "node -e \"process.stdout.write(process.env.AVACLAW_SHELL || '')\"",
    pty: true,
  });

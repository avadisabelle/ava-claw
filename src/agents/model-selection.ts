...
export function isCliProvider(provider: string, cfg?: AvaClawConfig): boolean {
  const normalized = normalizeProviderId(provider);
...
function resolveAllowedFallbacks(params: { cfg: AvaClawConfig; agentId?: string }): string[] {
  if (params.agentId) {

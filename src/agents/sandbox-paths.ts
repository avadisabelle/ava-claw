...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const avaClawTmpDir = path.resolve(resolvePreferredAvaClawTmpDir());
  if (!isPathInside(avaClawTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: avaClawTmpDir });
  return resolved;
}

...
function findTool(name: string, config: AvaClawConfig) {
  const allTools = createAvaClawTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: AvaClawConfig; env?: NodeJS.ProcessEnv }) {
...
describe("avaclaw tools runtime web metadata wiring", () => {

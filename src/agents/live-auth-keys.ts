...
const GOOGLE_LIVE_SINGLE_KEY = "AVACLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "AVACLAW_LIVE_ANTHROPIC_KEY",
    listVar: "AVACLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "AVACLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `AVACLAW_LIVE_${base}_KEY`;

...
function createOpenAiConfigWithResolvedApiKey(mergeMode = false): AvaClawConfig {
  return {
    models: {
...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureAvaClawModelsJson(validated.config);
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureAvaClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);

...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("avaclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("avaclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("avaclaw-sbx-");

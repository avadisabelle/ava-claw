...
const defaultTools = createAvaClawCodingTools({ senderIsOwner: true });
...
describe("createAvaClawCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: AvaClawConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createAvaClawCodingTools({
    ...
    const anthropicTools = createAvaClawCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: AvaClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createAvaClawCodingTools({
    ...
    const denied = createAvaClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createAvaClawCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });

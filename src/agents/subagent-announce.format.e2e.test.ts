...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/avadisabelle/ava-claw/issues/31298
    previousFastTestEnv = process.env.AVACLAW_TEST_FAST;
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.AVACLAW_TEST_FAST;
      return;
    }
    process.env.AVACLAW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // AVACLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("AvaClaw runtime context (internal):");

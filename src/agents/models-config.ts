...
import { planAvaClawModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: AvaClawConfig): {
  config: AvaClawConfig;
  sourceConfigForSecrets: AvaClawConfig;
} {
...
export async function ensureAvaClawModelsJson(
  config?: AvaClawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveAvaClawAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planAvaClawModelsJson({
      cfg,

...
import { markAvaClawExecEnv } from "../../infra/avaclaw-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "avaclaw.sandbox=1");
  args.push("--label", `avaclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `avaclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `avaclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markAvaClawExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "avaclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`avaclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`avaclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("avaclaw sandbox recreate --all");
}

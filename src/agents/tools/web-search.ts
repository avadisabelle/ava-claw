...
type WebSearchConfig = NonNullable<AvaClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: AvaClawConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  if (provider === "brave") {
    return {
      ...
      message: `web_search (brave) needs a Brave Search API key. Run \`${formatCliCommand("avaclaw configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
  ...
  return withTrustedWebSearchEndpoint(
    {
      ...
      init: {
        ...
        headers: {
          ...
          "HTTP-Referer": "https://avaclaw.ai",
          "X-Title": "AvaClaw Web Search",
        },

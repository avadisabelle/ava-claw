...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    avaclaw: process.env.AVACLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.avaclaw === undefined) {
      delete process.env.AVACLAW_GATEWAY_TOKEN;
    } else {
      process.env.AVACLAW_GATEWAY_TOKEN = envSnapshot.avaclaw;
    }
  ...
  it("uses AVACLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.AVACLAW_GATEWAY_TOKEN = "local-env-token";

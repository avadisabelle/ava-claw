...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): AvaClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as AvaClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): AvaClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as AvaClawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: AvaClawConfig, accountId = "media") {
    await handleTelegramAction(

...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "avaclaw" }),
                      },
                    },
                  ],
                },
    ...
    const result = await tool?.execute?.("call-1", { query: "latest avaclaw release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://avaclaw.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://avaclaw.ai/docs"]);

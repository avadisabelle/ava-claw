...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: AvaClawConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: AvaClawConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: AvaClawConfig) => string[];
  isConfigured?: (account: unknown, cfg: AvaClawConfig) => boolean;
  isEnabled?: (account: unknown, cfg: AvaClawConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "avaclaw.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ AVACLAW_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        avaclaw: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.AVACLAW_GATEWAY_TOKEN;
    const prevPassword = process.env.AVACLAW_GATEWAY_PASSWORD;
    delete process.env.AVACLAW_GATEWAY_TOKEN;
    delete process.env.AVACLAW_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.AVACLAW_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.AVACLAW_GATEWAY_PASSWORD;
      } else {
        process.env.AVACLAW_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/avaclaw-safe-bins"];
    const cfg: AvaClawConfig = {
      tools: {
        exec: {
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("avaclaw-sbx-browser-old\navaclaw-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "avaclaw-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "avaclaw-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("avaclaw-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("avaclaw-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "avaclaw-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "avaclaw-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-avaclaw.json");
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: AvaClawConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: AvaClawConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): AvaClawConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: AvaClawConfig = { session: { dmScope: "main" } };
  ...
  it("keeps channel security findings when SecretRef credentials are configured but unavailable", async () => {
    await withChannelSecurityStateDir(async () => {
      const sourceConfig: AvaClawConfig = {
        channels: {
          discord: {
            enabled: true,
      ...
      const resolvedConfig: AvaClawConfig = {
        channels: {
          discord: {
            enabled: true,
            groupPolicy: "allowlist",
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: AvaClawConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.AVACLAW_GATEWAY_TOKEN;
    process.env.AVACLAW_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: AvaClawConfig = {
  ...
  it("scores unrestricted hooks.allowedAgentIds by gateway exposure", async () => {
    ...
    } satisfies NonNullable<AvaClawConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: AvaClawConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: AvaClawConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "avaclaw.json"),
        execDockerRawFn: execDockerRawUnavailable,
      });
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "avaclaw.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.AVACLAW_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.AVACLAW_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };

...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("avaclaw-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.AVACLAW_STATE_DIR).toBe(path.join(tempHome.home, ".avaclaw"));
    ...
    expect(process.env.AVACLAW_STATE_DIR).toBe(previousStateDir);

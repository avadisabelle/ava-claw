...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".avaclaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.AVACLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$AVACLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via AVACLAW_STATE_DIR.

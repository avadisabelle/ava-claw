...
const { makeStorePath } = createCronStoreHarness({ prefix: "avaclaw-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses AVACLAW_HOME for tilde expansion", () => {
    vi.stubEnv("AVACLAW_HOME", "/srv/avaclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/avaclaw-home", "cron", "jobs.json"));
  });
});

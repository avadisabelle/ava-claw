...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "avaclaw-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".avaclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<AvaClawConfig> = {},
): AvaClawConfig {
  const base: AvaClawConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<AvaClawConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<AvaClawConfig>,

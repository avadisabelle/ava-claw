...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: AvaClawConfig) => Array<string | number> | undefined;
}): AvaClawConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: AvaClawConfig) => AvaClawConfig | Promise<AvaClawConfig>;
  applySet?: (
    cfg: AvaClawConfig,
    value: SecretInput,
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in AvaClaw config",
  ...
  const resolved = await promptSecretRefForOnboarding({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "AVACLAW_SECRET",
      envVarFormatError:
        'Use an env var name like "AVACLAW_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:

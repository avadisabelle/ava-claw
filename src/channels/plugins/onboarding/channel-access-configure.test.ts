...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: AvaClawConfig, policy: ChannelAccessPolicy) => AvaClawConfig;
  resolveAllowlist: (params: { cfg: AvaClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: AvaClawConfig; resolved: TResolved }) => AvaClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: AvaClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: AvaClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: AvaClawConfig, policy: ChannelAccessPolicy): AvaClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: AvaClawConfig, policy: ChannelAccessPolicy): AvaClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: AvaClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: AvaClawConfig; resolved: string[] }) => {
      calls.push("apply");

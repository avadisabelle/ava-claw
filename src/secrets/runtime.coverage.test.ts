...
function buildConfigForAvaClawTarget(entry: SecretRegistryEntry, envId: string): AvaClawConfig {
  const config = {} as AvaClawConfig;
  const refTargetPath =
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every avaclaw.json registry target when configured as active", async () => {
    const entries = listSecretTargetRegistryEntries().filter(
      (entry) => entry.configFile === "avaclaw.json",
    );
    for (const [index, entry] of entries.entries()) {
      const envId = `AVACLAW_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: buildConfigForAvaClawTarget(entry, envId),
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const [index, entry] of entries.entries()) {
      const envId = `AVACLAW_AUTH_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: {} as AvaClawConfig,
        env: { [envId]: expectedValue },
        agentDirs: ["/tmp/avaclaw-agent-main"],

...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): AvaClawConfig {
    const defaults: NonNullable<NonNullable<AvaClawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: AvaClawConfig) {

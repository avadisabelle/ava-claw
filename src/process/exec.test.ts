...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        AVACLAW_BASE_ENV: "base",
        AVACLAW_TO_REMOVE: undefined,
      },
      env: {
        AVACLAW_TEST_ENV: "ok",
      },
    ...
    expect(resolved.AVACLAW_BASE_ENV).toBe("base");
    expect(resolved.AVACLAW_TEST_ENV).toBe("ok");
    expect(resolved.AVACLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.AVACLAW_CLI).toBe(AVACLAW_CLI_ENV_VALUE);
  });

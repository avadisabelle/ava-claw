...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop avaclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@avaclaw_bot", { botUsername: "avaclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@avaclaw_bot", { botUsername: "avaclaw_bot" })).toBe(true);

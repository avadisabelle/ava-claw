...
const SLACK_REPLY_BUTTON_ACTION_ID = "avaclaw:reply_button";
const SLACK_REPLY_SELECT_ACTION_ID = "avaclaw:reply_select";
...
function buildButtonsBlock(raw: string, index: number): SlackBlock | null {
  ...
  return {
    type: "actions",
    block_id: `avaclaw_reply_buttons_${index}`,
...
function buildSelectBlock(raw: string, index: number): SlackBlock | null {
  ...
  return {
    type: "actions",
    block_id: `avaclaw_reply_select_${index}`,
    elements: [
      {
        type: "static_select",

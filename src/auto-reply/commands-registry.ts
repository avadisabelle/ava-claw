...
export function isCommandEnabled(cfg: AvaClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AvaClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AvaClawConfig),
...
export function getCommandDetection(_cfg?: AvaClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AvaClawConfig) {

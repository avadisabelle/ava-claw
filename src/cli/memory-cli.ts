    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["avaclaw memory status", "Show index and provider status."],
          ["avaclaw memory status --deep", "Probe embedding provider readiness."],
          ["avaclaw memory index --force", "Force a full reindex."],
          ['avaclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'avaclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["avaclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.avaclaw.ai/cli/memory")}\n`,
    );

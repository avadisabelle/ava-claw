...
${formatHelpExamples([
  ['avaclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['avaclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'avaclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'avaclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['avaclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'avaclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.avaclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.avaclaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['avaclaw agents set-identity --agent main --name "AvaClaw" --emoji "🦞"', "Set name + emoji."],
  ["avaclaw agents set-identity --agent main --avatar avatars/avaclaw.png", "Set avatar path."],
  [
    "avaclaw agents set-identity --workspace ~/.avaclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "avaclaw agents set-identity --identity-file ~/.avaclaw/workspace/IDENTITY.md --agent main",

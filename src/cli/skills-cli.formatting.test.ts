...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "avaclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "avaclaw-bundled-skills-test-"));
    process.env.AVACLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });

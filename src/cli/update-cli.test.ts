...
import type { AvaClawConfig, ConfigFileSnapshot } from "../config/types.avaclaw.js";
...
vi.mock("../infra/avaclaw-root.js", () => ({
  resolveAvaClawPackageRoot: vi.fn(),
}));
...
const { resolveAvaClawPackageRoot } = await import("../infra/avaclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/avaclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as AvaClawConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/avaclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveAvaClawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("avaclaw-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveAvaClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("avaclaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/avaclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "avaclaw-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("AvaClaw update status");
  });
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as AvaClawConfig,
        });
      },
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ["npm", "i", "-g", "avaclaw@latest", "--no-fund", "--no-audit", "--loglevel=error"],
      expect.any(Object),
    );
  });
  ...
  it("honors --tag override", async () => {
    ...
    expect(runGatewayUpdate).not.toHaveBeenCalled();
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ["npm", "i", "-g", "avaclaw@next", "--no-fund", "--no-audit", "--loglevel=error"],
      expect.any(Object),
    );
  });
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("avaclaw-update");
    const localAppData = createCaseDir("avaclaw-localappdata");
  ...
  it("uses AVACLAW_UPDATE_PACKAGE_SPEC for package updates", async () => {
    const tempDir = createCaseDir("avaclaw-update");
    mockPackageInstallStatus(tempDir);
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [
        ...
        "i",
        "-g",
        "http://10.211.55.2:8138/avaclaw-next.tgz",
        "--no-fund",
        "--no-audit",
        "--loglevel=error",
      ],
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("avaclaw-updated-root");
  ...
  it("updateCommand preserves invocation-relative service env overrides during refresh", async () => {
    ...
    await withEnvAsync(
      {
        AVACLAW_STATE_DIR: "./state",
        AVACLAW_CONFIG_PATH: "./config/avaclaw.json",
      },
      async () => {
        await updateCommand({});
      },
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          AVACLAW_STATE_DIR: path.resolve("./state"),
          AVACLAW_CONFIG_PATH: path.resolve("./config/avaclaw.json"),
        }),
        timeoutMs: 60_000,
      }),
    );
  ...
  it("updateCommand reuses the captured invocation cwd when process.cwd later fails", async () => {
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        cwd: root,
        env: expect.objectContaining({
          AVACLAW_STATE_DIR: path.resolve(originalCwd, "./state"),
        }),
        timeoutMs: 60_000,
      }),
    );
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ AVACLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.AVACLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("avaclaw-update-wizard");
    await withEnvAsync({ AVACLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);

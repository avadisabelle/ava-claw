...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("avaclaw-daemon-install-int-");
    configPath = path.join(tempHome, "avaclaw.json");
    process.env.HOME = tempHome;
    process.env.AVACLAW_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.AVACLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});

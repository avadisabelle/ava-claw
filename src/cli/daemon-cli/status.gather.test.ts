...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    AVACLAW_STATE_DIR: "/tmp/avaclaw-daemon",
    AVACLAW_CONFIG_PATH: "/tmp/avaclaw-daemon/avaclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.AVACLAW_STATE_DIR ?? "/tmp/avaclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.AVACLAW_CONFIG_PATH ?? `${stateDir}/avaclaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/avaclaw-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.AVACLAW_STATE_DIR = "/tmp/avaclaw-cli";
    process.env.AVACLAW_CONFIG_PATH = "/tmp/avaclaw-cli/avaclaw.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.AVACLAW_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.AVACLAW_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.AVACLAW_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "avaclaw-gateway" }],
        hints: [],
      },
      healthy: false,

...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "avaclaw";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "avaclaw"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "avaclaw" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option(
      "--driver <driver>",
      "Profile driver (avaclaw|extension|existing-session). Default: avaclaw",
    )
    .action(

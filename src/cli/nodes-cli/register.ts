    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["avaclaw nodes status", "List known nodes with live status."],
          ["avaclaw nodes pairing pending", "Show pending node pairing requests."],
          ['avaclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["avaclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.avaclaw.ai/cli/nodes")}\n`,
    );

...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      AVACLAW_GATEWAY_PORT: String(params.port),
      ...(params.token ? { AVACLAW_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.AVACLAW_STATE_DIR = "/tmp/avaclaw-cli-state";
    process.env.AVACLAW_CONFIG_PATH = "/tmp/avaclaw-cli-state/avaclaw.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        AVACLAW_PROFILE: "dev",
        AVACLAW_STATE_DIR: "/tmp/avaclaw-daemon-state",
        AVACLAW_CONFIG_PATH: "/tmp/avaclaw-daemon-state/avaclaw.json",
        AVACLAW_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.avaclaw.gateway.plist",
    });

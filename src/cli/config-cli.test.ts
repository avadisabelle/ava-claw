...
import type { ConfigFileSnapshot, AvaClawConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `avaclaw config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: AvaClawConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: AvaClawConfig, options?: { unsetPaths?: string[][] }) =>
...
function buildSnapshot(params: {
  resolved: AvaClawConfig;
  config: AvaClawConfig;
}): ConfigFileSnapshot {
  return {
    path: "/tmp/avaclaw.json",
    exists: true,
...
function setSnapshot(resolved: AvaClawConfig, config: AvaClawConfig) {
...
function withRuntimeDefaults(resolved: AvaClawConfig): AvaClawConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-avaclaw.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: {},
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: AvaClawConfig = {
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__AVACLAW_REDACTED__");
    });
  });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-avaclaw.json");
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/avaclaw.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });

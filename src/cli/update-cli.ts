...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update AvaClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["avaclaw update", "Update a source checkout (git)"],
        ["avaclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["avaclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["avaclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["avaclaw update --tag main", "One-off package install from GitHub main"],
        ["avaclaw update --dry-run", "Preview actions without changing anything"],
        ["avaclaw update --no-restart", "Update without restarting the service"],
        ["avaclaw update --json", "Output result as JSON"],
        ["avaclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["avaclaw update wizard", "Interactive update wizard"],
        ["avaclaw --update", "Shorthand for avaclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run avaclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.avaclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["avaclaw update status", "Show channel + version status."],
          ["avaclaw update status --json", "JSON output."],
          ["avaclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.avaclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {

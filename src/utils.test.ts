...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("avaclaw-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("avaclaw-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("avaclaw-lid-a-", async (first) => {
      await withTempDir("avaclaw-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.avaclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "avaclaw-config-dir-"));
    try {
  ...
  it("expands AVACLAW_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/avaclaw-home",
      AVACLAW_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/avaclaw-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers AVACLAW_HOME over HOME", () => {
    vi.stubEnv("AVACLAW_HOME", "/srv/avaclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/avaclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $AVACLAW_HOME prefix when AVACLAW_HOME is set", () => {
    vi.stubEnv("AVACLAW_HOME", "/srv/avaclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/avaclaw-home")}/.avaclaw/avaclaw.json`)).toBe(
      "$AVACLAW_HOME/.avaclaw/avaclaw.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $AVACLAW_HOME replacement when AVACLAW_HOME is set", () => {
    vi.stubEnv("AVACLAW_HOME", "/srv/avaclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/avaclaw-home")}/.avaclaw/avaclaw.json`),
    ).toBe("config: $AVACLAW_HOME/.avaclaw/avaclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/avaclaw", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "avaclaw"),
    );
  });
  ...
  it("prefers AVACLAW_HOME for tilde expansion", () => {
    vi.stubEnv("AVACLAW_HOME", "/srv/avaclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/avaclaw")).toBe(path.resolve("/srv/avaclaw-home", "avaclaw"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/avaclaw", env)).toBe(path.resolve("/srv/avaclaw-home", "avaclaw"));
  });

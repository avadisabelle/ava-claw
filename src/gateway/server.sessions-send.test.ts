...
const { createAvaClawTools } = await import("../agents/avaclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createAvaClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createAvaClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["AVACLAW_GATEWAY_PORT", "AVACLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.AVACLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.AVACLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("AVACLAW_CONFIG_PATH missing in gateway test environment");
      }

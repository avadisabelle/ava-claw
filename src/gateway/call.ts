...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.AVACLAW_GATEWAY_URL) ??
  ...
  const allowPrivateWs = process.env.AVACLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set AVACLAW_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: avaclaw doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__AVACLAW_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;

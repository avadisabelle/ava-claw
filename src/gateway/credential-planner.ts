...
type GatewaySecretDefaults = NonNullable<AvaClawConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${AVACLAW_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  return readGatewayEnv(env, ["AVACLAW_GATEWAY_TOKEN", "CLAWDBOT_GATEWAY_TOKEN"], includeLegacyEnv);
}
...
export function readGatewayPasswordEnv(
  ...
  return readGatewayEnv(
    env,
    ["AVACLAW_GATEWAY_PASSWORD", "CLAWDBOT_GATEWAY_PASSWORD"],
    includeLegacyEnv,
  );
}

...
export function resolveHooksConfig(cfg: AvaClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AvaClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-avaclaw-token"] === "string" ? req.headers["x-avaclaw-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-avaclaw-idempotency-key"]) ||

...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "AVACLAW_STATE_DIR",
  "AVACLAW_CONFIG_PATH",
  "AVACLAW_SKIP_BROWSER_CONTROL_SERVER",
  "AVACLAW_SKIP_GMAIL_WATCHER",
  "AVACLAW_SKIP_CANVAS_HOST",
  "AVACLAW_BUNDLED_PLUGINS_DIR",
  "AVACLAW_SKIP_CHANNELS",
  "AVACLAW_SKIP_PROVIDERS",
  "AVACLAW_SKIP_CRON",
  "AVACLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.AVACLAW_STATE_DIR = path.join(tempHome, ".avaclaw");
  delete process.env.AVACLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.AVACLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.AVACLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.AVACLAW_SKIP_CANVAS_HOST = "1";
  process.env.AVACLAW_SKIP_CHANNELS = "1";
  process.env.AVACLAW_SKIP_PROVIDERS = "1";
  process.env.AVACLAW_SKIP_CRON = "1";
  process.env.AVACLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.AVACLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "avaclaw-test-no-bundled-extensions")
    : "avaclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".avaclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".avaclaw-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__avaclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__avaclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["AVACLAW_GATEWAY_TOKEN"]);
  const prev = process.env.AVACLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.AVACLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.AVACLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();

...
import { createAvaClawTools } from "../agents/avaclaw-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-avaclaw-message-channel") ?? "");
  const accountId = getHeader(req, "x-avaclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-avaclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-avaclaw-thread-id")?.trim() || undefined;
  ...
  const allTools = createAvaClawTools({

...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("avaclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with AVACLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with AVACLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.AVACLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://avaclaw-gateway.ai:18789",
      onConnectError,
    });

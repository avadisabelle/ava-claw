...
import { type AvaClawConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: AvaClawConfig): string[] {
...
export function listAgentsForGateway(cfg: AvaClawConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: AvaClawConfig): string {
  return normalizeAgentId(resolveDefaultAgentId(cfg));
}

export function resolveSessionStoreKey(params: { cfg: AvaClawConfig; sessionKey: string }): string {
...
function resolveSessionStoreAgentId(cfg: AvaClawConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: AvaClawConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: AvaClawConfig): GatewaySessionsDefaults {

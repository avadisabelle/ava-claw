...
function cfg(input: Partial<AvaClawConfig>): AvaClawConfig {
  return input as AvaClawConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: AvaClawConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  AVACLAW_GATEWAY_TOKEN: "env-token",
  AVACLAW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves config-first token SecretRef even when AVACLAW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when AVACLAW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {

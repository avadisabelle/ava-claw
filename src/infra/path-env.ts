...
function candidateBinDirs(opts: EnsureAvaClawPathOpts): { prepend: string[]; append: string[] } {
...
/**
 * Best-effort PATH bootstrap so skills that require the `avaclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAvaClawCliOnPath(opts: EnsureAvaClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.AVACLAW_PATH_BOOTSTRAPPED)) {
    return;
  }

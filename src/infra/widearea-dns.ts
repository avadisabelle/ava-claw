...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AVACLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*avaclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "avaclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "avaclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "avaclaw-gw");
  ...
  records.push(`_avaclaw-gw._tcp IN PTR ${instanceLabel}._avaclaw-gw._tcp`);
  records.push(`${instanceLabel}._avaclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._avaclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; avaclaw-content-hash: ${contentHash}\n${contentBody}`;
}

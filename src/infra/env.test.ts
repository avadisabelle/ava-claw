...
describe("logAcceptedEnvOption", () => {
  it("logs accepted env options once with redaction and formatting", () => {
    ...
    withEnv(
      {
        ...
        AVACLAW_TEST_ENV: "  line one\nline two  ",
      },
      () => {
        logAcceptedEnvOption({
          key: "AVACLAW_TEST_ENV",
    ...
    expect(loggerMocks.info).toHaveBeenCalledWith("env: AVACLAW_TEST_ENV=<redacted> (test option)");
  });
    ...
    withEnv(
      {
        VITEST: "1",
        NODE_ENV: "development",
        AVACLAW_BLANK_ENV: "value",
      },
      () => {
        logAcceptedEnvOption({
          key: "AVACLAW_BLANK_ENV",

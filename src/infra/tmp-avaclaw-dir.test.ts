...
import { POSIX_AVACLAW_TMP_DIR, resolvePreferredAvaClawTmpDir } from "./tmp-avaclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredAvaClawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `avaclaw-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredAvaClawTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_AVACLAW_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredAvaClawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredAvaClawTmpDir", () => {
  it("prefers /tmp/avaclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_AVACLAW_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/avaclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_AVACLAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/avaclaw when /tmp/avaclaw is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/avaclaw when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/avaclaw exists but is not writable", () => {
    const accessSync = vi.fn((target: string) => {
      if (target === POSIX_AVACLAW_TMP_DIR) {
  ...
  it("falls back when /tmp/avaclaw is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/avaclaw is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/avaclaw is group/other writable", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ mode: 0o40777 })) });
  });

  it("repairs existing /tmp/avaclaw permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_AVACLAW_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_AVACLAW_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_AVACLAW_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/avaclaw after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_AVACLAW_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_AVACLAW_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_AVACLAW_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_AVACLAW_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback AvaClaw temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "avaclaw");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredAvaClawTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_AVACLAW_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback AvaClaw temp dir/);
  });
});

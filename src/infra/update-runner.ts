...
const DEFAULT_PACKAGE_NAME = "avaclaw";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["avaclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "avaclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("avaclaw doctor", doctorArgv, gitRoot, { AVACLAW_UPDATE_IN_PROGRESS: "1" }),
    );

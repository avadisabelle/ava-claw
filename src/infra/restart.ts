...
export function triggerAvaClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.AVACLAW_SYSTEMD_UNIT,
      process.env.AVACLAW_PROFILE,
    );
  ...
  const label =
    process.env.AVACLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.AVACLAW_PROFILE);

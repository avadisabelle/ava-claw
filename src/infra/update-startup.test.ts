...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["AVACLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.AVACLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveAvaClawPackageRoot } = await import("./avaclaw-root.js"));
    ...
    vi.mocked(resolveAvaClawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveAvaClawPackageRoot).mockResolvedValue("/opt/avaclaw");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/avaclaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });

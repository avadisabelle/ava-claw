...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: AvaClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: AvaClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: AvaClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: AvaClawConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: AvaClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: AvaClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: AvaClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: AvaClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();

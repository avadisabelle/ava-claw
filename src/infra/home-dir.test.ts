...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers AVACLAW_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for AVACLAW_HOME", () => {
    const result = resolveRequiredHomeDir(
      { AVACLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when AVACLAW_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ AVACLAW_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { AVACLAW_HOME: "/srv/avaclaw-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/avaclaw-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/avaclaw-home " },
      expected: "/srv/avaclaw-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/avaclaw-home/docs"));
  });

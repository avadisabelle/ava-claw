...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __avaclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__avaclawDiagnosticEventsState) {
    globalStore.__avaclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__avaclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: AvaClawConfig): boolean {

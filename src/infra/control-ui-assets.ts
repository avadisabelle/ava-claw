...
import { resolveAvaClawPackageRoot, resolveAvaClawPackageRootSync } from "./avaclaw-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveAvaClawPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "avaclaw" + dist/control-ui/index.html
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (fs.existsSync(pkgJsonPath)) {
        try {
          ...
          if (parsed.name === "@avadisabelle/ava-claw" || parsed.name === "avaclaw") {
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/avaclaw -> .../dist/index.js)

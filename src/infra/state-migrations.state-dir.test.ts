...
async function makeTempRoot() {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "avaclaw-state-dir-"));
  tempRoot = root;
  return root;
}
...
describe("legacy state dir auto-migration", () => {
  it("follows legacy symlink when it points at another legacy dir (clawdbot -> moltbot)", async () => {
    ...
    const targetMarker = path.join(root, ".avaclaw", "marker.txt");
  ...
  it("skips state-dir migration when AVACLAW_STATE_DIR is explicitly set", async () => {
    const root = await makeTempRoot();
    const legacyDir = path.join(root, ".clawdbot");
    fs.mkdirSync(legacyDir, { recursive: true });

    const result = await autoMigrateLegacyStateDir({
      env: { AVACLAW_STATE_DIR: path.join(root, "custom-state") } as NodeJS.ProcessEnv,
      homedir: () => root,
    });

...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AvaClawConfig | null = null;
  ...
  get(key: string, cfg: AvaClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AvaClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: AvaClawConfig): void {

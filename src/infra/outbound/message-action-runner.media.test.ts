...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred AvaClaw tmp root", async () => {
      const tmpRoot = resolvePreferredAvaClawTmpDir();
      ...
      try {
        ...
        const hostTmpOutsideAvaClaw = path.join(os.tmpdir(), "outside-avaclaw", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",

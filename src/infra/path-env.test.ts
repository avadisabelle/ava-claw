...
let ensureAvaClawCliOnPath: typeof import("./path-env.js").ensureAvaClawCliOnPath;

describe("ensureAvaClawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "AVACLAW_PATH_BOOTSTRAPPED",
    "AVACLAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureAvaClawCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/avaclaw-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "avaclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureAvaClawCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling avaclaw exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.AVACLAW_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.AVACLAW_PATH_BOOTSTRAPPED = "1";
    ensureAvaClawCliOnPath({
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "avaclaw");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.AVACLAW_PATH_BOOTSTRAPPED;
      delete process.env.AVACLAW_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.AVACLAW_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/avaclaw-path/case-linuxbrew");

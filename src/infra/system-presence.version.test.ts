...
describe("system-presence version fallback", () => {
  ...
  it("uses runtime VERSION when AVACLAW_VERSION is not set", async () => {
  ...
  it("prefers AVACLAW_VERSION over runtime VERSION", async () => {
    await expectSelfVersion(
      {
  ...
  it("still prefers runtime VERSION over AVACLAW_SERVICE_VERSION when AVACLAW_VERSION is blank", async () => {
    await expectSelfVersion(
      {
        AVACLAW_VERSION: " ",
        AVACLAW_SERVICE_VERSION: "2.4.6-service",
  ...
  it("uses runtime VERSION when AVACLAW_VERSION and AVACLAW_SERVICE_VERSION are blank", async () => {
    await expectSelfVersion(
      {
        AVACLAW_VERSION: " ",
        AVACLAW_SERVICE_VERSION: "\t",

...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/avaclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/avaclaw-state", "devices"),
      pendingPath: path.join("/tmp/avaclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/avaclaw-state", "devices", "paired.json"),
    });
  });

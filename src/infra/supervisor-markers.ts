const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "AVACLAW_LAUNCHD_LABEL"],
  systemd: ["AVACLAW_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["AVACLAW_WINDOWS_TASK_NAME"],
} as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.AVACLAW_SERVICE_MARKER?.trim();
    const serviceKind = env.AVACLAW_SERVICE_KIND?.trim();

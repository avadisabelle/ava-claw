...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevAvaClawHome = process.env.AVACLAW_HOME;
    const prevHome = process.env.HOME;
    process.env.AVACLAW_HOME = "/srv/avaclaw-home";
    process.env.HOME = "/home/other";
    const avaClawHome = path.join(path.resolve("/srv/avaclaw-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", avaClawHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevAvaClawHome === undefined) {
        delete process.env.AVACLAW_HOME;
      } else {
        process.env.AVACLAW_HOME = prevAvaClawHome;
      }

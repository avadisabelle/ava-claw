...
const TEMP_MEDIA_PREFIX = "avaclaw-media-";
...
function createGroqAudioConfig(): AvaClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): AvaClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): AvaClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: AvaClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredAvaClawTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        AVACLAW_AGENT_DIR: isolatedAgentDir,

...
describe("canvas host state dir defaults", () => {
  it("uses AVACLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("avaclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("AvaClaw Canvas");
      } finally {
        await handler.close();
      }
    });

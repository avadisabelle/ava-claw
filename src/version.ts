...
declare const __AVACLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "@avadisabelle/ava-claw";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["AVACLAW_VERSION"],
      runtimeVersion,
      env["AVACLAW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current AvaClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __AVACLAW_VERSION__ === "string" ? __AVACLAW_VERSION__ : undefined,
  bundledVersion: process.env.AVACLAW_BUNDLED_VERSION,
});

...
import { ensureAvaClawExecMarkerOnProcess } from "./infra/avaclaw-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "avaclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "avaclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.AVACLAW_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.AVACLAW_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.AVACLAW_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.AVACLAW_NODE_OPTIONS_READY = "1";
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `AvaClaw ${VERSION} (${commit})` : `AvaClaw ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[avaclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[avaclaw] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[avaclaw] ${parsed.error}`);
      process.exit(2);
    }

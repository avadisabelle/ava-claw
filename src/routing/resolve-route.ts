...
function listAgents(cfg: AvaClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: AvaClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<AvaClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: AvaClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: AvaClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AvaClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<AvaClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  AvaClawConfig,
  {
    bindingsRef: AvaClawConfig["bindings"];
    agentsRef: AvaClawConfig["agents"];
    sessionRef: AvaClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: AvaClawConfig): Map<string, ResolvedAgentRoute> {

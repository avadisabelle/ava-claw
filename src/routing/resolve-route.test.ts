...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: AvaClawConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: AvaClawConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/avaclaw-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<AvaClawConfig["bindings"]>[number];

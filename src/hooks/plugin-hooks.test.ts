...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "avaclaw-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.AVACLAW_BUNDLED_HOOKS_DIR;
    process.env.AVACLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });

  afterEach(() => {
    clearInternalHooks();
    if (previousBundledHooksDir === undefined) {
      delete process.env.AVACLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.AVACLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  function createConfig(enabled: boolean): AvaClawConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("avaclaw-plugin");
  ...
  it("does not treat Claude hooks.json bundles as AvaClaw hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".avaclaw", "extensions", "claude-bundle");

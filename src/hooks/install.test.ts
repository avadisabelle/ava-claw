...
const fixtureRoot = path.join(os.tmpdir(), `avaclaw-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@avadisabelle/ava-claw-test-hooks",
      version: "0.0.1",
      avaclaw: { hooks: params.hooks },
...
describe("installHooksFromPath", () => {
  ...
  it("rejects hook pack entries that traverse outside package directory", async () => {
    ...
    expectPathInstallFailureContains(result, "avaclaw.hooks entry escapes package directory");
  });
  ...
  it("rejects hook pack entries that escape via symlink", async () => {
    ...
    expectPathInstallFailureContains(
      result,
      "avaclaw.hooks entry resolves outside package directory",
    );
  });
});
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@avadisabelle/ava-claw-test-hooks@0.0.1",
              name: "@avadisabelle/ava-claw-test-hooks",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@avadisabelle/ava-claw-test-hooks@0.0.1",
      hooksDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@avadisabelle/ava-claw-test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@avadisabelle/ava-claw-test-hooks@0.0.1",
    });
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@avadisabelle/ava-claw-test-hooks@0.0.2-beta.1",
      name: "@avadisabelle/ava-claw-test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@avadisabelle/ava-claw-test-hooks@beta"');
    }
  });
});

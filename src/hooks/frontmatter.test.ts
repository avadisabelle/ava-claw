...
`;
    ...
    expect(parsed.avaclaw.emoji).toBe("💾");
    expect(parsed.avaclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.avaclaw.emoji).toBe("📝");
    expect(parsed.avaclaw.events).toEqual(["command"]);
    expect(parsed.avaclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.avaclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"avaclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"avaclaw": {"events": ["test"]}}');
  });
...
describe("resolveAvaClawMetadata", () => {
  it("extracts avaclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAvaClawMetadata(frontmatter);
  ...
  it("returns undefined when avaclaw key is missing", () => {
    const frontmatter = {
...
homepage: https://docs.avaclaw.ai/automation/hooks#session-memory
metadata:
  {
    "avaclaw":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with AvaClaw" }],
      },
  }
---
...
`;
    ...
    const avaclaw = resolveAvaClawMetadata(frontmatter);
    expect(avaclaw).toBeDefined();
    expect(avaclaw?.emoji).toBe("💾");
    expect(avaclaw?.events).toEqual(["command:new", "command:reset"]);
    expect(avaclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(avaclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const avaclaw = resolveAvaClawMetadata(frontmatter);
    expect(avaclaw?.emoji).toBe("disk");
    expect(avaclaw?.events).toEqual(["command:new"]);
  });
});

...
This directory contains hooks that ship with AvaClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
2. Restart gateway: `pkill -9 -f 'avaclaw.*gateway' && pnpm avaclaw gateway`
3. Enable the hook: `avaclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.avaclaw.ai/automation/hooks

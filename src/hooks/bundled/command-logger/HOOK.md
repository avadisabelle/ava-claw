...
homepage: https://docs.avaclaw.ai/automation/hooks#command-logger
metadata:
  {
    "avaclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.avaclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.avaclaw/logs/commands.log ~/.avaclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/avaclaw`:
   ```
   /home/username/.avaclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok

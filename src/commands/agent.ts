...
function prependInternalEventContext(
  ...
  if (body.includes("AvaClaw runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("avaclaw agents list")}" to see configured agents.`,
      );
    }
  }

...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAvaClawAgentDir: vi.fn().mockReturnValue("/tmp/avaclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/avaclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/avaclaw-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/avaclaw-dev/avaclaw.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAvaClawAgentDir: mocks.resolveAvaClawAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/avaclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAvaClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/avaclaw-dev/avaclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/avaclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/avaclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/avaclaw-agent-custom");

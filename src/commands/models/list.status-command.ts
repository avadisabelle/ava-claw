...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAvaClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("avaclaw models auth setup-token")}\` or \`${formatCliCommand("avaclaw configure")}\`.`
          : `Run \`${formatCliCommand("avaclaw configure")}\` or set an API key env var.`;

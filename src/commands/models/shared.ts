...
export async function loadValidConfigOrThrow(): Promise<AvaClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: AvaClawConfig) => AvaClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: AvaClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AvaClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("avaclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}

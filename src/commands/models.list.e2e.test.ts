...
const ensureAvaClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveAvaClawAgentDir = vi.fn().mockReturnValue("/tmp/avaclaw-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensureAvaClawModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveAvaClawAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureAvaClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureAvaClawModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureAvaClawModelsJson).toHaveBeenCalled();
    expect(ensureAvaClawModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });

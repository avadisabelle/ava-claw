...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: AvaClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: AvaClawConfig) => { changed: boolean; next: AvaClawConfig };
  defaultModel: string;
  overrideConfig: AvaClawConfig;
  alreadyDefaultConfig: AvaClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: AvaClawConfig) => config,
      applyDefaultConfig: (config: AvaClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: AvaClawConfig = { agents: { defaults: {} } };

...
import { AvaClawSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: AvaClawConfig): {
  config: AvaClawConfig;
  removed: string[];
} {
  const parsed = AvaClawSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: AvaClawConfig): void {

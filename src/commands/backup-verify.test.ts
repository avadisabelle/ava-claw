...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-avaclaw-backup";
...
describe("backupVerifyCommand", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("avaclaw-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    const stateDir = path.join(tempHome.home, ".avaclaw");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-backup-verify-out-"));
    try {
      await fs.writeFile(path.join(stateDir, "avaclaw.json"), JSON.stringify({}), "utf8");
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-avaclaw-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.avaclaw",
            archivePath: `${rootName}/payload/posix/tmp/.avaclaw`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "avaclaw-backup-traversal-",
  ...
  it("fails when archive paths contain backslashes", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "avaclaw-backup-backslash-",
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".avaclaw");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-backup-verify-out-"));
    try {
      process.env.AVACLAW_CONFIG_PATH = configPath;
      await fs.writeFile(
        configPath,
        JSON.stringify({
          agents: {
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.avaclaw/payload.txt`;
    await withBrokenArchiveFixture(
      {
        tempPrefix: "avaclaw-backup-duplicate-manifest-",
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "avaclaw-backup-duplicate-payload-",
        manifestAssetArchivePath: payloadArchivePath,
        payloads: [

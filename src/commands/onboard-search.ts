...
function rawKeyValue(config: AvaClawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: AvaClawConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: AvaClawConfig, provider: SearchProvider): AvaClawConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: AvaClawConfig, result: AvaClawConfig): AvaClawConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with avaclaw configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — AvaClaw will store a reference instead of the API key.",

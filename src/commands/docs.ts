...
const SEARCH_TOOL = "https://docs.avaclaw.ai/mcp.SearchAvaClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.avaclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('avaclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.avaclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('avaclaw docs "your query"')}`);
    }
    return;
  }

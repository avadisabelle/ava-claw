...
import type { OAuthCredentials } from "@avadisabelle/ava-pi-ai";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under AVACLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("avaclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-oauth-sync-"));
    ...
    process.env.AVACLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-oauth-nosync-"));
    process.env.AVACLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside AVACLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-oauth-external-"));
    process.env.AVACLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* AVACLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to AVACLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("avaclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);

...
type EnvSnapshot = {
  HOME?: string;
  AVACLAW_HOME?: string;
  AVACLAW_STATE_DIR?: string;
  AVACLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    AVACLAW_HOME: process.env.AVACLAW_HOME,
    AVACLAW_STATE_DIR: process.env.AVACLAW_STATE_DIR,
    AVACLAW_OAUTH_DIR: process.env.AVACLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: AvaClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: AvaClawConfig) {
...
async function runStateIntegrityText(cfg: AvaClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "avaclaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.AVACLAW_HOME = tempHome;
    process.env.AVACLAW_STATE_DIR = path.join(tempHome, ".avaclaw");
    delete process.env.AVACLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.AVACLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when AVACLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.AVACLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: AvaClawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints avaclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: AvaClawConfig = {};
    ...
    expect(text).toMatch(/avaclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/avaclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /avaclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );

...
export function normalizeCompatibilityConfigValues(cfg: AvaClawConfig): {
  config: AvaClawConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: AvaClawConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as AvaClawConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as AvaClawConfig["browser"],
    };
    changes.push(

...
function resolveMode(cfg: AvaClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("AvaClaw doctor");

  const root = await resolveAvaClawPackageRoot({
  ...
  let cfg: AvaClawConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("avaclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("avaclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("avaclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("avaclaw config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("avaclaw config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("avaclaw doctor --fix")}" to apply changes.`);
  }

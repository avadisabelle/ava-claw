...
function resolveAgentName(cfg: AvaClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: AvaClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AvaClawConfig): AgentSummary[] {

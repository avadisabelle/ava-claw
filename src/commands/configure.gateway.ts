...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "AVACLAW_GATEWAY_TOKEN",
          placeholder: "AVACLAW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like AVACLAW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. AvaClaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: AvaClaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.avaclaw.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );

...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByAvaClaw ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByAvaClaw ? "installed · " : "";
  ...
  runtime.log(theme.heading("AvaClaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`avaclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("avaclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("avaclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("avaclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("avaclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.avaclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.avaclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("avaclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("avaclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("avaclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("avaclaw gateway probe")}`);
  }
}

...
import { readConfigFileSnapshot, writeConfigFile, type AvaClawConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("avaclaw config set gateway.auth.mode token")} or ${formatCliCommand("avaclaw config set gateway.auth.mode password")}.`,
  ].join(" ");
}

...
export async function runNonInteractiveOnboardingLocal(params: {
        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `avaclaw gateway run` or `avaclaw agent --local`.",
              ]
            : [`Run \`${formatCliCommand("avaclaw gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("avaclaw gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("avaclaw configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.avaclaw.ai/tools/web`,
    );
  }
}

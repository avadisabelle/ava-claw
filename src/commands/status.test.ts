...
beforeAll(() => {
  envSnapshot = captureEnv(["AVACLAW_PROFILE"]);
  process.env.AVACLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/avaclaw-root.js", () => ({
  resolveAvaClawPackageRoot: vi.fn().mockResolvedValue("/tmp/avaclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/avaclaw/pnpm-lock.yaml",
      markerPath: "/tmp/avaclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.avaclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.avaclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("avaclaw status --all") ||
          line.includes("avaclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("AVACLAW_GATEWAY_TOKEN", "abcd1234", async () => {

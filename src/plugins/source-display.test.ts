...
function createPluginSourceRoots() {
  ...
  const globalRoot = path.resolve(path.sep, "Users", "x", ".avaclaw", "extensions");
  const workspaceRoot = path.resolve(path.sep, "Users", "x", "ws", ".avaclaw", "extensions");
  return {
    stock: stockRoot,
    global: globalRoot,
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    ...
    const homeDir = path.resolve(path.sep, "tmp", "avaclaw-home");
    const roots = withEnv(
      {
        AVACLAW_BUNDLED_PLUGINS_DIR: path.join(ignoredHome, "ignored-bundled"),
        AVACLAW_STATE_DIR: path.join(ignoredHome, "ignored-state"),
        HOME: ignoredHome,
      },
      () =>
        resolvePluginSourceRoots({
          env: {
            ...process.env,
            HOME: homeDir,
            AVACLAW_BUNDLED_PLUGINS_DIR: "~/bundled",
    ...
    expect(roots).toEqual({
      ...
      workspace: path.join(homeDir, "ws", ".avaclaw", "extensions"),
    });
  });
});

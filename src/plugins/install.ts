...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing avaclaw.extensions; update the plugin package to include avaclaw.extensions (for example ["./dist/index.js"]). See https://docs.avaclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-avaclaw-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_AVACLAW_EXTENSIONS: "missing_avaclaw_extensions",
  EMPTY_AVACLAW_EXTENSIONS: "empty_avaclaw_extensions",
...
function ensureAvaClawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_AVACLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json avaclaw.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_AVACLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Bundle "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "avaclaw security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Bundle "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "avaclaw security audit --deep" after install.`,
    );
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureAvaClawExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureAvaClawExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from avaclaw.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-avaclaw"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "avaclaw security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "avaclaw security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "avaclaw-npm-pack-",
    spec,
    timeoutMs,

...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core AvaClaw release numbers.\n\n`;
...
function stripWorkspaceAvaclawDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.avaclaw !== "workspace:*") {
    return false;
  }
  delete devDeps.avaclaw;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceAvaclawDevDependency(pkg);

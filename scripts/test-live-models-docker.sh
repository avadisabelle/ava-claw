...
docker run --rm -t \
  ...
  -e AVACLAW_LIVE_MODELS="${AVACLAW_LIVE_MODELS:-${CLAWDBOT_LIVE_MODELS:-modern}}" \
  -e AVACLAW_LIVE_PROVIDERS="${AVACLAW_LIVE_PROVIDERS:-${CLAWDBOT_LIVE_PROVIDERS:-}}" \
  -e AVACLAW_LIVE_MAX_MODELS="${AVACLAW_LIVE_MAX_MODELS:-${CLAWDBOT_LIVE_MAX_MODELS:-48}}" \
  -e AVACLAW_LIVE_MODEL_TIMEOUT_MS="${AVACLAW_LIVE_MODEL_TIMEOUT_MS:-${CLAWDBOT_LIVE_MODEL_TIMEOUT_MS:-}}" \
  -e AVACLAW_LIVE_REQUIRE_PROFILE_KEYS="${AVACLAW_LIVE_REQUIRE_PROFILE_KEYS:-${CLAWDBOT_LIVE_REQUIRE_PROFILE_KEYS:-}}" \
  -v "$ROOT_DIR":/src:ro \

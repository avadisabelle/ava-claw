...
#
# Suggested cron: */30 * * * * /home/admin/avaclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send AvaClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., avaclaw-alerts)
...
STATE_FILE="$HOME/.avaclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via AvaClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use avaclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via AvaClaw to $NOTIFY_PHONE..."
            avaclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! AvaClaw is down. Run: ssh l36 '~/avaclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1

...
# AvaClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://avaclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one AvaClaw."
...
GUM_VERSION="${AVACLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 AvaClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://avaclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$AVACLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.avaclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_avaclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.avaclaw-* "$npm_root"/avaclaw 2>/dev/null || true
}

extract_avaclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_avaclaw_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/avaclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/avaclaw"|"/usr/local/bin/avaclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/avaclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale avaclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing avaclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing AvaClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_avaclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "AvaClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*avaclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_avaclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_avaclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "AvaClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an avaclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "AVACLAW_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "AVACLAW_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "AVACLAW_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "AVACLAW_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "AVACLAW_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "AVACLAW_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "AVACLAW_BETA" "CLAWDBOT_BETA"
map_legacy_env "AVACLAW_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "AVACLAW_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "AVACLAW_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "AVACLAW_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "AVACLAW_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "AVACLAW_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${AVACLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${AVACLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((AVACLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${AVACLAW_NO_ONBOARD:-0}
NO_PROMPT=${AVACLAW_NO_PROMPT:-0}
DRY_RUN=${AVACLAW_DRY_RUN:-0}
INSTALL_METHOD=${AVACLAW_INSTALL_METHOD:-}
AVACLAW_VERSION=${AVACLAW_VERSION:-latest}
USE_BETA=${AVACLAW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/avaclaw"
GIT_DIR=${AVACLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${AVACLAW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${AVACLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${AVACLAW_VERBOSE:-0}"
VERIFY_INSTALL="${AVACLAW_VERIFY_INSTALL:-0}"
AVACLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
AvaClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://avaclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/avaclaw)
...
Environment variables:
  AVACLAW_INSTALL_METHOD=git|npm
  AVACLAW_VERSION=latest|next|main|<semver>|<spec>
  AVACLAW_BETA=0|1
  AVACLAW_GIT_DIR=...
  AVACLAW_GIT_UPDATE=0|1
  AVACLAW_NO_PROMPT=1
  AVACLAW_VERIFY_INSTALL=1
  AVACLAW_DRY_RUN=1
  AVACLAW_NO_ONBOARD=1
  AVACLAW_VERBOSE=1
  AVACLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://avaclaw.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://avaclaw.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://avaclaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://avaclaw.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://avaclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected AvaClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a AvaClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_avaclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"avaclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://avaclaw.ai/install.sh | bash"
}
...
ensure_avaclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/avaclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/avaclaw" ]]; then
        ln -sf "$npm_root/avaclaw/dist/entry.js" "${npm_bin}/avaclaw"
        ui_info "Created avaclaw bin link at ${npm_bin}/avaclaw"
    fi
    return 0
}

# Check for existing AvaClaw installation
check_existing_avaclaw() {
    if [[ -n "$(type -P avaclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing AvaClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make avaclaw show as \"command not found\" in new terminals."
...
warn_avaclaw_not_found() {
    ui_warn "Installed, but avaclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t avaclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named avaclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_avaclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P avaclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/avaclaw" ]]; then
        echo "${npm_bin}/avaclaw"
        return 0
    fi
...
install_avaclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/avaclaw/avaclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing AvaClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing AvaClaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning AvaClaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building AvaClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/avaclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/avaclaw"
    ui_success "AvaClaw wrapper installed to \$HOME/.local/bin/avaclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view avaclaw dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:avaclaw/avaclaw#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_avaclaw() {
    local package_name="avaclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            AVACLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="avaclaw"
        else
            AVACLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${AVACLAW_VERSION}" ]]; then
        AVACLAW_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${AVACLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${AVACLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing AvaClaw v${resolved_version}"
    else
        ui_info "Installing AvaClaw (${AVACLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${AVACLAW_VERSION}")"

    if ! install_avaclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_avaclaw_paths
        install_avaclaw_npm "${install_spec}"
    fi

    if [[ "${AVACLAW_VERSION}" == "latest" && "${package_name}" == "avaclaw" ]]; then
        if ! resolve_avaclaw_bin &> /dev/null; then
            ui_warn "npm install avaclaw@latest failed; retrying avaclaw@next"
            cleanup_npm_avaclaw_paths
            install_avaclaw_npm "avaclaw@next"
        fi
    fi

    ensure_avaclaw_bin_link || true

    ui_success "AvaClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${AVACLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_avaclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (avaclaw not on PATH yet)"
        warn_avaclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${AVACLAW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.avaclaw/workspace-${profile}"
    else
        echo "${HOME}/.avaclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${AVACLAW_CONFIG_PATH:-$HOME/.avaclaw/avaclaw.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run avaclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but avaclaw not on PATH; skipping onboarding"
        warn_avaclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run avaclaw onboard to retry"
        return
    }
}
...
resolve_avaclaw_version() {
    local version=""
    local raw_version_output=""
    local claw="${AVACLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v avaclaw &> /dev/null; then
        claw="$(command -v avaclaw)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_avaclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/avaclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/avaclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: avaclaw not on PATH yet"
        warn_avaclaw_not_found
        return 1
    fi

    run_quiet_step "Checking AvaClaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: avaclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_avaclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found AvaClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set AVACLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_avaclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing AvaClaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g avaclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g avaclaw 2>/dev/null || true
        ...
        install_avaclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/avaclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/avaclaw"
    ...
    AVACLAW_BIN="$(resolve_avaclaw_bin || true)"
    ...
    installed_version=$(resolve_avaclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 AvaClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 AvaClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/avaclaw"
        ui_kv "Update command" "avaclaw update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://avaclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running avaclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                AVACLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                AVACLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                AVACLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run avaclaw doctor and avaclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run avaclaw onboard later"
        else
            local config_path="${AVACLAW_CONFIG_PATH:-$HOME/.avaclaw/avaclaw.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (avaclaw not on PATH yet)"
                    warn_avaclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run avaclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v avaclaw &> /dev/null; then
        local claw="${AVACLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_avaclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (avaclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if AVACLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: avaclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${AVACLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi

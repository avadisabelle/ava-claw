...
# Build and bundle AvaClaw into a minimal .app we can open.
# Outputs to dist/AvaClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/AvaClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="AvaClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.avaclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/avaclaw/avaclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/AvaClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AvaClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AvaClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/AvaClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/AvaClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/AvaClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/AvaClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/AvaClaw/Resources/AvaClaw.icns" "$APP_ROOT/Contents/Resources/AvaClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/AvaClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"

echo "📦 Copying model catalog"
MODEL_CATALOG_SRC="$ROOT_DIR/node_modules/@avadisabelle/ava-pi-ai/dist/models.generated.js"
...
echo "📦 Copying AvaClawKit resources"
AVACLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AvaClawKit_AvaClawKit.bundle"
if [ -d "$AVACLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AvaClawKit_AvaClawKit.bundle"
  cp -R "$AVACLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AvaClawKit_AvaClawKit.bundle"
else
  echo "WARN: AvaClawKit resource bundle not found at $AVACLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running AvaClaw"
killall -q AvaClaw 2>/dev/null || true

...
- regenerates apps/ios/AvaClaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${AVACLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid AVACLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid AVACLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid AVACLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid AVACLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing AVACLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(AVACLAW_URL_SLASH)$(AVACLAW_URL_SLASH)#g'
)"
...
AVACLAW_CODE_SIGN_STYLE = Automatic
AVACLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
AVACLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
AVACLAW_APP_BUNDLE_ID = ai.avaclaw.client
AVACLAW_SHARE_BUNDLE_ID = ai.avaclaw.client.share
AVACLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.avaclaw.client.activitywidget
AVACLAW_WATCH_APP_BUNDLE_ID = ai.avaclaw.client.watchkitapp
AVACLAW_WATCH_EXTENSION_BUNDLE_ID = ai.avaclaw.client.watchkitapp.extension
AVACLAW_APP_PROFILE =
AVACLAW_SHARE_PROFILE =
AVACLAW_PUSH_TRANSPORT = relay
AVACLAW_PUSH_DISTRIBUTION = official
AVACLAW_URL_SLASH = /
AVACLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
AVACLAW_PUSH_APNS_ENVIRONMENT = production
EOF

#!/bin/bash
# Setup AvaClaw Auth Management System
...
echo "=== AvaClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'avaclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  AvaClaw message: Send warning via AvaClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/avaclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/avaclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/avaclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now avaclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status avaclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"

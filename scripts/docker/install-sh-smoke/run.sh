...
INSTALL_URL="${AVACLAW_INSTALL_URL:-https://avaclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${AVACLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${AVACLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="avaclaw"
PACKAGE_NAME="${AVACLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (AVACLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${AVACLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${AVACLAW_INSTALL_LATEST_OUT:-}"
fi

...
IMAGE_NAME="avaclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="avaclaw-net-e2e-$$"
GW_NAME="avaclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "AVACLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "AVACLAW_SKIP_CHANNELS=1" \
  -e "AVACLAW_SKIP_GMAIL_WATCHER=1" \
  -e "AVACLAW_SKIP_CRON=1" \
  -e "AVACLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \

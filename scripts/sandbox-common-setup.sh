...
BASE_IMAGE="${BASE_IMAGE:-avaclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-avaclaw-sandbox-common:bookworm-slim}"
...
AVACLAW_DOCKER_BUILD_USE_BUILDX="${AVACLAW_DOCKER_BUILD_USE_BUILDX:-0}"
AVACLAW_DOCKER_BUILD_CACHE_FROM="${AVACLAW_DOCKER_BUILD_CACHE_FROM:-}"
AVACLAW_DOCKER_BUILD_CACHE_TO="${AVACLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${AVACLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${AVACLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${AVACLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${AVACLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${AVACLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=avaclaw.sandbox=1)
NOTE

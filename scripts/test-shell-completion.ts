 ...
 * `avaclaw update`. Use it to verify the completion installation flow
...
\`avaclaw update\`, \`avaclaw doctor\`, and \`avaclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# AvaClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`avaclaw completion --install\` later to enable.`));
    return;
  }

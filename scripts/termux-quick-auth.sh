...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/avaclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "AvaClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/avaclaw/scripts/mobile-reauth.sh'" --id avaclaw-auth
        ;;
    *)

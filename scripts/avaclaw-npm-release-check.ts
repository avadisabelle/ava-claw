...
const EXPECTED_REPOSITORY_URL = "https://github.com/avadisabelle/ava-claw";
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`avaclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `avaclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}

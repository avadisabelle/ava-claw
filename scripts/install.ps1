# AvaClaw Installer for Windows (PowerShell)
# Usage: iwr -useb https://avaclaw.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://avaclaw.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\avaclaw",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 AvaClaw Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one AvaClaw.$NC" -Level info
    Write-Host ""
}
...
function Install-AvaClawGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing AvaClaw from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/avaclaw/avaclaw.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\avaclaw\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\avaclaw.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install AvaClaw from git to $GitDir" -Level info
        } else {
            Install-AvaClawGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install AvaClaw via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-AvaClawNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'avaclaw onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 AvaClaw installed successfully!" -Level success
}

#!/usr/bin/env bash
# ClawDock - Docker helpers for AvaClaw
# Inspired by Simon Willison's "Running AvaClaw in Docker"
# https://til.simonwillison.net/llms/avaclaw-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/avaclaw/avaclaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for AvaClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/avaclaw"
  "${HOME}/workspace/avaclaw"
  "${HOME}/projects/avaclaw"
  "${HOME}/dev/avaclaw"
  "${HOME}/code/avaclaw"
  "${HOME}/src/avaclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found AvaClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/avaclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ AvaClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/avaclaw/avaclaw.git ~/avaclaw"
    echo "  cd ~/avaclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^AVACLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d avaclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart avaclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f avaclaw-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec avaclaw-gateway \
    bash -c 'echo "alias avaclaw=\"./avaclaw.mjs\"" > /tmp/.bashrc_avaclaw && bash --rcfile /tmp/.bashrc_avaclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm avaclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build avaclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "AVACLAW_GATEWAY_TOKEN=$token" avaclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" avaclaw-gateway \
    bash -c './avaclaw.mjs config set gateway.remote.token "$TOKEN" && ./avaclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec avaclaw-gateway \
    bash -c "./avaclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart avaclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm avaclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec avaclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'avaclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for AvaClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (avaclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to avaclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.avaclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'avaclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'avaclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.avaclaw.ai${_CLR_RESET}"
  echo ""
}

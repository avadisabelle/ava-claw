...
type DriverMode = "token" | "webhook" | "avaclaw";
...
function resolveStateDir(): string {
  const override = process.env.AVACLAW_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.AVACLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".avaclaw");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver avaclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) AvaClaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|avaclaw> Driver transport mode (default: token)\n" +
    ...
    "  --avaclaw-bin <path>        AvaClaw CLI binary for driver=avaclaw (default: avaclaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  AVACLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  AVACLAW_DISCORD_SMOKE_DRIVER\n" +
    "  AVACLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  AVACLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  AVACLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  AVACLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  AVACLAW_DISCORD_SMOKE_AGENT\n" +
    "  AVACLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  AVACLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  AVACLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  AVACLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  AVACLAW_DISCORD_SMOKE_AVACLAW_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.AVACLAW_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.AVACLAW_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.AVACLAW_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.AVACLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.AVACLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.AVACLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.AVACLAW_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.AVACLAW_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.AVACLAW_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.AVACLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.AVACLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.AVACLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const avaclawBin =
    resolveArg("--avaclaw-bin") || process.env.AVACLAW_DISCORD_SMOKE_AVACLAW_BIN || "avaclaw";
...
async function avaclawCliJson<T>(params: { avaclawBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.avaclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`avaclaw ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithAvaclaw(params: {
  avaclawBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await avaclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    avaclawBin: params.avaclawBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "avaclaw") {
    return await readMessagesWithAvaclaw({
      avaclawBin: params.args.avaclawBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `avaclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await avaclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("avaclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "avaclaw"
            ? await readMessagesWithAvaclaw({
                avaclawBin: args.avaclawBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from AvaClaw.`,
        diagnostics: {
          bindingCandidates: [
            {

...
const extensionRestartMetadataFiles = new Set(["avaclaw.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.AVACLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.AVACLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[avaclaw] ${message}\n`);
};

const runAvaClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["avaclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });

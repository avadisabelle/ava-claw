...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["avaclaw"]
        let text = "hey avaclaw"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey avaclaw do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["avaclaw"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "avaclaw", start: 0.2, duration: 0.1, range: transcript.range(of: "avaclaw")),

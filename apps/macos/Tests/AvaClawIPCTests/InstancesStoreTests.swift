...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: AvaClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: AvaClawProtocol.AnyCodable] = [
            "presence": .init([AvaClawProtocol.AnyCodable(entry)]),
        ]
        let payload = AvaClawProtocol.AnyCodable(payloadMap)

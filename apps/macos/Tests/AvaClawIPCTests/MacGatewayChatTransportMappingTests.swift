...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: AvaClawProtocol.AnyCodable(["ok": AvaClawProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: AvaClawProtocol.AnyCodable(["ok": AvaClawProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = AvaClawProtocol.AnyCodable([
            "runId": AvaClawProtocol.AnyCodable("run-1"),
            "sessionKey": AvaClawProtocol.AnyCodable("main"),
            "state": AvaClawProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: AvaClawProtocol.AnyCodable(["a": AvaClawProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)

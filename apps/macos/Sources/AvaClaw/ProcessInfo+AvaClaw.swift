...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.avaclaw.mac`) even if the shipped
    ...
    {
        if environment["AVACLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "avaclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "avaclaw.nixMode") { return true }

...
private let webChatSwiftLogger = Logger(subsystem: "ai.avaclaw", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "avaclaw.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: AvaClawChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> AvaClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(AvaClawChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            AvaClawChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return AvaClawChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AvaClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AvaClawGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: AvaClawProtocol.ModelChoice) -> AvaClawChatModelChoice {
        AvaClawChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AvaClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AvaClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AvaClawChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: AvaClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AvaClawChatView>) -> NSViewController
    {

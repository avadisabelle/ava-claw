...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AvaClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.avaclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AvaClawKit.AnyCodable]()) {
                $0[$1.key] = AvaClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AvaClawProtocol.AnyCodable?) -> [String: AvaClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AvaClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AvaClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AvaClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("avaclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("avaclaw.control.agent")
}

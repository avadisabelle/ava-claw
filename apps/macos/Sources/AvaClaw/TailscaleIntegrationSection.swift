...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.avaclaw/avaclaw.json. Prefer AVACLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.avaclaw/avaclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.avaclaw/avaclaw.json. Restart the gateway to apply."
        }

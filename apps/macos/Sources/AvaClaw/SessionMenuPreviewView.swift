...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.avaclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AvaClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AvaClawSessionPreviewEntry,
    ...
    private static func cache(payload: AvaClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AvaClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AvaClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AvaClawKit.AnyCodable]) -> [AvaClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AvaClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AvaClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AvaClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AvaClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AvaClawChatMessage) -> String? {

...
enum AvaClawLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = AvaClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AvaClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AvaClawLogging.bootstrapIfNeeded()
        let label = AvaClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AvaClawOSLogHandler: AppLogLevelBackedHandler {
...
struct AvaClawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AvaClawLogging.parseLabel(self.label)

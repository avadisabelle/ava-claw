...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["AVACLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/avaclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("avaclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("avaclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/avaclaw/ (rolling or stdout), or nil if none exist.

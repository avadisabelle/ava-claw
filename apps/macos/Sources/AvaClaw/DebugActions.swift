...
enum DebugActions {
    private static let verboseDefaultsKey = "avaclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "avaclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AvaClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "AvaClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AvaClawPaths.configURL
        guard

...
enum CommandResolver {
    private static let projectRootDefaultsKey = "avaclaw.gatewayProjectRootPath"
    private static let helperName = "avaclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let avaclawEntry = root.appendingPathComponent("avaclaw.mjs").path
        if FileManager().isReadableFile(atPath: avaclawEntry) { return avaclawEntry }
        let binEntry = root.appendingPathComponent("bin/avaclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/avaclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let avaclawPaths = self.avaclawManagedPaths(home: home)
        if !avaclawPaths.isEmpty {
            extras.insert(contentsOf: avaclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + avaclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func avaclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".avaclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func avaclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAvaClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("avaclaw.mjs").path,
            root.appendingPathComponent("bin/avaclaw.js").path,
        ]
    ...
    static func hasAnyAvaClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.avaclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func avaclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let avaclawPath = self.projectAvaClawExecutable(projectRoot: root) {
            return [avaclawPath, subcommand] + extraArgs
        }
        if let avaclawPath = self.avaclawExecutable(searchPaths: searchPaths) {
            return [avaclawPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "avaclaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            avaclaw entrypoint missing (looked for dist/index.js or avaclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real avaclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/avaclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v avaclaw >/dev/null 2>&1; then
          CLI="$(command -v avaclaw)"
          avaclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/avaclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/avaclaw.mjs"
            node "$PRJ/avaclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/avaclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/avaclaw.js"
            node "$PRJ/bin/avaclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent avaclaw"
          pnpm --silent avaclaw \(quotedArgs);
        else
          echo "avaclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? AvaClawConfigFile.loadDict()

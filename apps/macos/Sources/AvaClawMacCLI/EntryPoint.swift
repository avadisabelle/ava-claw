...
struct AvaClawMacCLI {
    static func main() async {
        ...
        default:
            fputs("avaclaw-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      avaclaw-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      avaclaw-mac discover [--timeout <ms>] [--json] [--include-local]
      avaclaw-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      avaclaw-mac connect
      avaclaw-mac connect --url ws://127.0.0.1:18789 --json
      avaclaw-mac discover --timeout 3000 --json
      avaclaw-mac wizard --mode local
    """)
}

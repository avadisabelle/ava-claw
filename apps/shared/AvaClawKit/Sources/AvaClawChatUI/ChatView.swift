...
@MainActor
public struct AvaClawChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: AvaClawChatViewModel
    ...
    public init(
        viewModel: AvaClawChatViewModel,
    ...
    private var visibleMessages: [AvaClawChatMessage] {
        let base: [AvaClawChatMessage]
    ...
    private func mergeToolResults(in messages: [AvaClawChatMessage]) -> [AvaClawChatMessage] {
    ...
    private func isToolResultMessage(_ message: AvaClawChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: AvaClawChatMessage) -> Bool {
    ...
    private func primaryText(in message: AvaClawChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: AvaClawChatMessage) -> Bool {
    ...
    private func toolCalls(in message: AvaClawChatMessage) -> [AvaClawChatMessageContent] {
    ...
    private func inlineToolResults(in message: AvaClawChatMessage) -> [AvaClawChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: AvaClawChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: AvaClawChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(AvaClawChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)

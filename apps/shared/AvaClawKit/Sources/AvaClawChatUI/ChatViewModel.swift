...
private let chatUILogger = Logger(subsystem: "ai.avaclaw", category: "AvaClawChatUI")

@MainActor
@Observable
public final class AvaClawChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [AvaClawChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [AvaClawChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [AvaClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [AvaClawChatPendingToolCall] = []
    public private(set) var sessions: [AvaClawChatSessionEntry] = []
    private let transport: any AvaClawChatTransport
    private var sessionDefaults: AvaClawChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: AvaClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any AvaClawChatTransport,
    ...
    public var sessionChoices: [AvaClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [AvaClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: AvaClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [AvaClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AvaClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: AvaClawChatMessage) -> AvaClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> AvaClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return AvaClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: AvaClawChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [AvaClawChatMessage],
        incoming: [AvaClawChatMessage]) -> [AvaClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [AvaClawChatMessage]) -> [AvaClawChatMessage] {
        var result: [AvaClawChatMessage] = []
    ...
    private static func dedupeKey(for message: AvaClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [AvaClawChatMessageContent] = [
            AvaClawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> AvaClawChatAttachmentPayload in
            AvaClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> AvaClawChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = AvaClawChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: AvaClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: AvaClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: AvaClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AvaClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> AvaClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)

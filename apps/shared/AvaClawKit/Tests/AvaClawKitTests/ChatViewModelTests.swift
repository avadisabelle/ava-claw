...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> AvaClawChatHistoryPayload
{
    AvaClawChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> AvaClawChatSessionEntry {
    AvaClawChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> AvaClawChatSessionEntry
{
    AvaClawChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> AvaClawChatModelChoice {
    AvaClawChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [AvaClawChatHistoryPayload],
    sessionsResponses: [AvaClawChatSessionsListResponse] = [],
    modelResponses: [[AvaClawChatModelChoice]] = [],
    ...
    -> (TestChatTransport, AvaClawChatViewModel)
{
...
private func sendUserMessage(_ vm: AvaClawChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, AvaClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [AvaClawChatHistoryPayload]
    private let sessionsResponses: [AvaClawChatSessionsListResponse]
    private let modelResponses: [[AvaClawChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<AvaClawChatTransportEvent>
    private let continuation: AsyncStream<AvaClawChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<AvaClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> AvaClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? AvaClawChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [AvaClawChatAttachmentPayload]) async throws -> AvaClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return AvaClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> AvaClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? AvaClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: AvaClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = AvaClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = AvaClawChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: AvaClawChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = AvaClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: AvaClawChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(AvaClawChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == AvaClawChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = AvaClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: AvaClawChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = AvaClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = AvaClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = AvaClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { AvaClawChatViewModel(sessionKey: "main", transport: transport) }

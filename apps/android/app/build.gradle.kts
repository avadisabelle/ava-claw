...
val androidStoreFile = providers.gradleProperty("AVACLAW_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("AVACLAW_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("AVACLAW_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("AVACLAW_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set AVACLAW_ANDROID_STORE_FILE, " +
            "AVACLAW_ANDROID_STORE_PASSWORD, AVACLAW_ANDROID_KEY_ALIAS, and " +
            "AVACLAW_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/AvaClawKit/Sources/AvaClawKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.avaclaw.app"
        minSdk = 31
        targetSdk = 36
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName = "avaclaw-$versionName-$buildType.apk"

...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__AVACLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<AvaClawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.avaclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = AvaClawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> AvaClawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return AvaClawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> AvaClawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> AvaClawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return AvaClawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> AvaClawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = AvaClawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: AvaClawTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: AvaClawAgentEventPayload.self
            ) else {
                continue
            }

...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AvaClawContactsSearchParams) async throws -> AvaClawContactsSearchPayload {
        ...
        return AvaClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AvaClawContactsAddParams) async throws -> AvaClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AvaClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AvaClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AvaClawContactPayload {
        AvaClawContactPayload(

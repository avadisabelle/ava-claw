...
final class RemindersService: RemindersServicing {
    func list(params: AvaClawRemindersListParams) async throws -> AvaClawRemindersListPayload {
        ...
        let payload: [AvaClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return AvaClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AvaClawRemindersAddParams) async throws -> AvaClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AvaClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AvaClawRemindersAddPayload(reminder: payload)
    }

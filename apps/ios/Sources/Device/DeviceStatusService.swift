...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> AvaClawDeviceStatusPayload {
        ...
        return AvaClawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> AvaClawDeviceInfoPayload {
        ...
        return AvaClawDeviceInfoPayload(
    ...
    private func batteryStatus() -> AvaClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: AvaClawBatteryState = switch device.batteryState {
        ...
        return AvaClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> AvaClawThermalStatusPayload {
        let state: AvaClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return AvaClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> AvaClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return AvaClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }

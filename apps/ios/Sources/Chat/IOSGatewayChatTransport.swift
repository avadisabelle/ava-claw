...
struct IOSGatewayChatTransport: AvaClawChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.avaclaw", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> AvaClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AvaClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AvaClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(AvaClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [AvaClawChatAttachmentPayload]) async throws -> AvaClawChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AvaClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(AvaClawChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(AvaClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AvaClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AvaClawChatEventPayload.self)
                        {

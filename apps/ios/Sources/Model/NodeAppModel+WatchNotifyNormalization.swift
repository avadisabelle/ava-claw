...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: AvaClawWatchNotifyParams) -> AvaClawWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [AvaClawWatchAction]?,
        kind: String?,
        promptId: String?) -> [AvaClawWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> AvaClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AvaClawWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                AvaClawWatchAction(id: "approve", label: "Approve"),
                AvaClawWatchAction(id: "decline", label: "Decline", style: "destructive"),
                AvaClawWatchAction(id: "open_phone", label: "Open iPhone"),
                AvaClawWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            AvaClawWatchAction(id: "done", label: "Done"),
            AvaClawWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            AvaClawWatchAction(id: "open_phone", label: "Open iPhone"),
            AvaClawWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: AvaClawWatchRisk?,
        priority: AvaClawNotificationPriority?) -> AvaClawWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: AvaClawNotificationPriority?,
        risk: AvaClawWatchRisk?) -> AvaClawNotificationPriority?
    {

...
final class MotionService: MotionServicing {
    func activities(params: AvaClawMotionActivityParams) async throws -> AvaClawMotionActivityPayload {
        ...
        let mapped: [AvaClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return AvaClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: AvaClawPedometerParams) async throws -> AvaClawPedometerPayload {
        ...
        let payload: AvaClawPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = AvaClawPedometerPayload(

...
@MainActor
final class AvaClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.avaclaw.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.avaclaw.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.avaclaw.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "avaclaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "avaclaw.watch.promptId"
    static let sessionKeyKey = "avaclaw.watch.sessionKey"
    static let actionPrimaryIDKey = "avaclaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "avaclaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "avaclaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "avaclaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "avaclaw.watch.action.primary"
    static let actionSecondaryIdentifier = "avaclaw.watch.action.secondary"
    static let actionIdentifierPrefix = "avaclaw.watch.action."
    static let actionIDKeyPrefix = "avaclaw.watch.action.id."
    static let actionLabelKeyPrefix = "avaclaw.watch.action.label."
    static let categoryPrefix = "avaclaw.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: AvaClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> AvaClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AvaClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "AvaClaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [AvaClawWatchAction]) -> [UNNotificationAction] {
...
@main
struct AvaClawApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(AvaClawAppDelegate.self) private var appDelegate
...
extension AvaClawApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("AvaClaw: installing uncaught exception handler")

...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.avaclaw.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to AvaClaw", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to AvaClaw gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to AvaClaw.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "AvaClawShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "AvaClaw is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "AvaClaw Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("avaclaw-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in

...
import type { AvaClawConfig, PluginRuntime } from "avaclaw/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): AvaClawConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-bb-media-"));
  tempDirs.push(dir);
  return dir;
}

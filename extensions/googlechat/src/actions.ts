import type {
  ...
  AvaClawConfig,
} from "avaclaw/plugin-sdk/googlechat";
import {
  createActionGate,
  extractToolSend,
  jsonResult,
...
function listEnabledAccounts(cfg: AvaClawConfig) {
  return listEnabledGoogleChatAccounts(cfg).filter(
    (account) => account.enabled && account.credentialSource !== "none",
  );
}

function isReactionsEnabled(accounts: ReturnType<typeof listEnabledAccounts>, cfg: AvaClawConfig) {

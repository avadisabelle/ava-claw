import type { AnyAgentTool, AvaClawPluginApi } from "avaclaw/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: AvaClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<AvaClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},

  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/avaclaw",
      },
    },
    session: { store: "/tmp/avaclaw-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bavaclaw\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "avaclaw: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "AvaClaw" },
          },
        },
        eventPatch: {

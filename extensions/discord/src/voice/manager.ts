...
function resolveVoiceTtsConfig(params: { cfg: AvaClawConfig; override?: TtsConfig }): {
  cfg: AvaClawConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredAvaClawTmpDir(), "discord-voice-"));

...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn(() => "/tmp/avaclaw-discord-process-test-sessions.json"),
}));
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/avaclaw-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/avaclaw-discord-process-test-sessions.json" },
      },
    });

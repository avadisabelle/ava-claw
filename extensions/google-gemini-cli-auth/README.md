# Google Gemini CLI Auth (AvaClaw plugin)
...
```bash
avaclaw plugins enable google-gemini-cli-auth
```
...
```bash
avaclaw models auth login --provider google-gemini-cli --set-default
```
...
- `AVACLAW_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `AVACLAW_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`

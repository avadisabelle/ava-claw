...
import type { AvaClawPluginConfigSchema } from "avaclaw/plugin-sdk/acpx";
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    if (
      fs.existsSync(path.join(cursor, "avaclaw.plugin.json")) &&
...
export function createAcpxPluginConfigSchema(): AvaClawPluginConfigSchema {
  return {

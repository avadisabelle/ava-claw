...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-avaclaw-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-avaclaw-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.avaclawDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.avaclawDiffsError = "true";

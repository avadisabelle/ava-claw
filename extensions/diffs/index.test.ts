...
import type { AvaClawPluginApi } from "avaclaw/plugin-sdk/diffs";
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<AvaClawPluginApi["registerHttpRoute"]>[0];
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<AvaClawPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as AvaClawPluginApi);

...
import type { AvaClawPluginApi } from "avaclaw/plugin-sdk/device-pair";
import {
...
} from "avaclaw/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: AvaClawPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.AVACLAW_GATEWAY_PORT?.trim()) ??
...
function resolveAuthLabel(cfg: AvaClawPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: AvaClawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the AvaClaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the AvaClaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",

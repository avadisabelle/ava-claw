...
// Prefix for AvaClaw-generated action IDs to scope our handler
const AVACLAW_ACTION_PREFIX = "avaclaw:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${AVACLAW_ACTION_PREFIX}`);

  // Handle AvaClaw modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({

...
import { resolvePreferredAvaClawTmpDir } from "../../../src/infra/tmp-avaclaw-dir.js";
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub AVACLAW_STATE_DIR.
    // Also keep it outside the AvaClaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["AVACLAW_STATE_DIR"]);
    process.env.AVACLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "avaclaw-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredAvaClawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default AvaClaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default AvaClaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));

...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__avaclaw_channel_id";
...
type InteractionCallbackConfig = Pick<AvaClawConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "avaclaw-mattermost-interactions").update(botToken).digest("hex");
}

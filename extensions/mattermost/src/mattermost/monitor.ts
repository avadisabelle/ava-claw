...
export async function monitorMattermostProvider(opts: MonitorMattermostOpts = {}): Promise<void> {
  ...
  if (slashEnabled) {
    try {
      ...
      // Use the *runtime* listener port when available (e.g. `avaclaw gateway run --port <port>`).
      // The gateway sets AVACLAW_GATEWAY_PORT when it boots, but the config file may still contain
      // a different port.
      const envPortRaw = process.env.AVACLAW_GATEWAY_PORT?.trim();
      ...
      authorizeButtonClick: async ({ payload, post }) => {
        ...
        return {
          ...
          response: {
            ...
            ephemeral_text: `AvaClaw ignored this action for ${decision.roomLabel}.`,
          },
        };
      },

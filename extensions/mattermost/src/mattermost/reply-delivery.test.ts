...
import type { AvaClawConfig } from "avaclaw/plugin-sdk/mattermost";
...
describe("deliverMattermostReplyPayload", () => {
  it("passes agent-scoped mediaLocalRoots when sending media paths", async () => {
    const previousStateDir = process.env.AVACLAW_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-mm-state-"));
    ...
    try {
      ...
      const cfg = {} satisfies AvaClawConfig;
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.AVACLAW_STATE_DIR = previousStateDir;
      }

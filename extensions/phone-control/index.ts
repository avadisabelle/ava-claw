...
import type { AvaClawPluginApi, AvaClawPluginService } from "avaclaw/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: AvaClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AvaClawPluginApi["config"]): string[] {
...
export default function register(api: AvaClawPluginApi) {
  ...
  const timerService: AvaClawPluginService = {

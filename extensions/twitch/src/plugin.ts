...
import type { AvaClawConfig } from "avaclaw/plugin-sdk/twitch";
import { buildChannelConfigSchema } from "avaclaw/plugin-sdk/twitch";
 ...
 * for AvaClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: AvaClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: AvaClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: AvaClawConfig): boolean => {

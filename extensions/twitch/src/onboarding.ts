...
import {
  ...
  type WizardPrompter,
} from "avaclaw/plugin-sdk/twitch";
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: AVACLAW_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: AvaClawConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var AVACLAW_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.AVACLAW_TWITCH_ACCESS_TOKEN?.trim();

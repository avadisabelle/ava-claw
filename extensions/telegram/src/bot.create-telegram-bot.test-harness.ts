...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/avaclaw-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
const DEFAULT_TELEGRAM_TEST_CONFIG: AvaClawConfig = {
  agents: {
    defaults: {

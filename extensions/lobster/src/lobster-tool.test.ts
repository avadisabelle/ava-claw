...
import type { AvaClawPluginApi, AvaClawPluginToolContext } from "avaclaw/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<AvaClawPluginApi> = {}): AvaClawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<AvaClawPluginToolContext> = {}): AvaClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "avaclaw-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: AvaClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }

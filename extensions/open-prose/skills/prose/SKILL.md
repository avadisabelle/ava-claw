...
metadata: { "avaclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == AvaClaw `sessions_spawn`
- **File I/O** == AvaClaw `read`/`write`
- **Remote fetch** == AvaClaw `web_fetch` (or `exec` with curl when POST is required)

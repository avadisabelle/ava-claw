...
vi.mock("avaclaw/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("avaclaw/plugin-sdk/diagnostics-otel")>(
    "avaclaw/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { AvaClawPluginServiceContext } from "avaclaw/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "avaclaw/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/avaclaw-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): AvaClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(
      telemetryState.histograms.get("avaclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("avaclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("avaclaw.webhook.processed");
    expect(spanNames).toContain("avaclaw.message.processed");
    expect(spanNames).toContain("avaclaw.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["avaclaw.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("avaclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "avaclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["avaclaw.reason"]).toBe("string");
    expect(String(attrs?.["avaclaw.reason"])).not.toContain(

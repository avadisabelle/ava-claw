import {
  ...
  type AvaClawConfig,
  type RuntimeEnv,
} from "avaclaw/plugin-sdk/nextcloud-talk";
...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as AvaClawConfig);
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as AvaClawConfig,
    surface: CHANNEL_ID,
  });
  const useAccessGroups =
    (config.commands as Record<string, unknown> | undefined)?.useAccessGroups !== false;
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as AvaClawConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as AvaClawConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as AvaClawConfig);

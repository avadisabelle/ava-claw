...
function resolveAvaClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.AVACLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set AVACLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveAvaClawRoot(), "dist", "extensionAPI.js");

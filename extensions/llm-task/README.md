...
`avaclaw.invoke --each`) without adding new AvaClaw code per workflow.
...
This extension depends on AvaClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** AvaClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.avaclaw/extensions` as a standalone plugin directory.

  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.AVACLAW_HOME;
        delete env.AVACLAW_STATE_DIR;
        delete env.AVACLAW_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "avaclaw.mjs");

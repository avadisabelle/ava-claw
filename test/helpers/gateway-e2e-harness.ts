...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `avaclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".avaclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          AVACLAW_SKIP_PROVIDERS: "1",
          AVACLAW_SKIP_GMAIL_WATCHER: "1",
          AVACLAW_SKIP_CRON: "1",
          AVACLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
          AVACLAW_SKIP_CANVAS_HOST: "1",
          AVACLAW_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },

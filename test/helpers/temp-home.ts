...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked AVACLAW_HOME.
  delete process.env.AVACLAW_HOME;
  process.env.AVACLAW_STATE_DIR = path.join(base, ".avaclaw");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "avaclaw-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".avaclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
